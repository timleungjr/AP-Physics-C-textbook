...
work. The object's kinetic energy is constant, and angular velocity
$\bm\omega$ and speed $v$ also remains constant. The object, under this
centripetal force, will remain in the same uniform circular motion forever, or
until the net force changes. In other words, the \emph{rotational state} of the
object does not change. We conclude that the rotational of an object is not
determined by merely what forces are acting it.
...
at a point some position $\bm r$ away from the centre of rotation, call a
\textbf{pivot}, or a \textbf{fulcrum}.
...
defined as the cross product of $\bm F$ and $\bm r$. The SI unit for torque is
a \emph{newton metre} (\si{\newton\metre}).
...
\begin{important-equation}
  \tau=rF_\perp=r_\perp F=rF\sin\phi
  \label{torque2}
\end{important-equation}
where $\phi$ is the angle between the force and position vectors;
$F_\perp=F\sin\phi$ is the component of force orthogonal to the position
vector, and $r_\perp=r\sin\phi$ is the component of the position vector that is
orthogonal to the line of action of the force vector. By treating the problem
as a one-dimensional vector, $\tau$ is positive if it is counterclockwise, and
...
\end{figure}
At any moment in time, it has a linear momentum of $\bm p=m\bm v$, but it also
has an \textbf{angular momentum} $\bm L$, defined in a way analagous to the
...
\begin{important-equation}
  I=mr^2\quad\text{(single particle)}
\end{important-equation}
...
\begin{important-equation}
  I=\sum_{i=1}^N m_ir_i^2\quad\text{($N$ discrete particles)}
\end{important-equation}
...
becomes an integral:
\begin{important-equation}
  I=\int r^2\dl m\quad\text{(continuous distribution of mass)}
\end{important-equation}
...
\begin{example}
  A slender rod of length $L$ and mass $M$ is hinged at one end, as shown in
  the diagram below. Assuming that the density of the rod is uniform, what is
  ...
  \begin{center}
    \begin{tikzpicture}
      \draw[mass] rectangle (10,.8);
      \fill (0,.4) circle (.07) node[above]{\scriptsize pivot};
      \draw[thick,|<->|] (0,1)--(10,1) node[midway,above]{$L$};
      \fill (0,.4) circle (.05) node[above]{\scriptsize cm};
    \end{tikzpicture}
  \end{center}
  \textbf{Solution:}
\end{example}
...
\begin{example}
  A slender rod with a density that varies linearly ($\gamma=\alpha x$) with
  length, total length $L$ is hinged at one end, as shown in the diagram below.
  Assuming that the density of the rod is uniform, what is its moment of
  inertia about its pivot?
  \begin{center}
    \begin{tikzpicture}
      \draw[mass] rectangle (10,.8);
      \fill (0,.4) circle (.07) node[above]{\scriptsize pivot};
...
changing the rotational state of an object.

When we compare the exressions of linear momentum to that of angular momentum in
...
\begin{align*}
  ...
  \bm L &= I\bm\omega
\end{align*}
Just as $\bm p$ describes the \emph{translational} state of motion of a
translational system, $\bm L$ describes its \emph{rotational} state of a
rotating system.

Now we can extend our knowledge of the laws of motion for translational motion
to formulate the the laws of motion for rorational motion.
...
$\bm a_\text{cm}=\bm 0$, and therefore $\bm v_\text{cm}=\text{constant}$.

Likewise, an object is in \textbf{rotational equilibrium} when the net
\emph{torque} acting on it is zero:
\begin{important-equation}
  \bm\tau_\text{net}=\bm0\quad\text{rotational equalibrium}
\end{important-equation}
...
momentum does not change with time, or $\dot{\bm L}=0$. For an object with
constant moment of inertia $I$, this means that $\bm\alpha=\bm 0$, and
$\bm\omega=\text{constant}$.

\begin{example}
  A skater extends her arms (both arms!), holding a \SI{2.0}{\kilo\gram} mass
  in each hand. She is rotating about a vertical axis at a given rate. She
  brings her arms inward toward her body in such a way that the distance of
  each mass from the axis changes from \SI{1.0}{\metre} to \SI{.50}\metre. Her
  rate of rotation (neglecting her own mass) will?
\end{example}
...
Likewise, the second law of motion for rotational motion is similar, but with
torque $\bm\tau$ replacing force $\bm F$, and angular momentum $\bm L$
replacing linear momentum $\bm p$:
\begin{important-equation}
...
\begin{important-equation}
  ...
  \quad\text{(constant moment of inertia)}
\end{important-equation}




\section{Angular Momentum in Rectilinear Motion}
...
Fig.~\ref{fig:straight-line-angular-momentum}, a mass $m$ travels along a
straight line at constant velocity (uniform motion).
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
    \draw[dashed] (-6,0)--(5,0);
    \draw[vector,red] (-5,0)--+(1.5,0) node[above]{$\bm v$};
    \draw[vector,red,dotted,rotate around={66.4:(-5,0)}]
    (-5,0)--+(.6,0) node[above]{$v_\perp=v\sin\theta$};
    \draw[mass] (-5,0) circle (.2) node{$m$} node[above=4]{1};
    \draw[vector,blue] (0,-2)--(-5,0)
    node[midway,below,rotate=-atan{2/5}]{$\bm r_1$}
    node[pos=.78,above,black]{$\theta$};

    \draw[vector,red] (0,0)--+(1.5,0) node[above]{$\bm v$};
    \draw[mass] circle (.2) node{$m$} node[above=4]{2};
    \draw[vector,blue] (0,-2)--(0,0) node[midway,left]{$\bm r_2$};

    \draw[vector,red] (3,0)--+(1.5,0) node[above]{$\bm v$};
    \draw[mass] (3,0) circle (.2) node{$m$} node[above=4]{3};
    \draw[vector,blue] (0,-2)--(3,0)
    node[midway,below,rotate=atan{2/4}]{$\bm r_3$}
    node[pos=.77,above,black]{$\phi$};
    \draw[vector,red,dotted,rotate around={-acos(2/3):(3,0)}]
    (3,0)--+(1,0) node[below]{$v_\perp=v\sin\phi$};
...
\end{figure}
The angular momentum around (arbitrarily chosen) point $P$, however, is not
zero. At position 1, since $\bm v_\perp\neq 0$, there is clearly a clockwise
angular momentum about point P, with a magnitude of:
\begin{equation*}
  L_1=r_1mv_\perp=r_1mv\sin\theta
\end{equation*}
As the object moves to position 2, the clockwise angular momentum about point P
is very easy to calculate. But since $r_2=r_1\sin\theta$, we conclude that
angular momentum at position 2 is the same as at position 1:
\begin{equation*}
  L_2={\color{red}r_2}mv={\color{red}(r_1\sin\theta)}mv=L_1
\end{equation*}
Not surprisingly, we can apply the same logic to find the angular momentum at
position 3:
\begin{equation*}
  L_3=r_3mv_\perp=r_3mv\sin\phi
\end{equation*}
Again, relating $r_3$ to $r_2$, and from $r_2$ to $r_1$, we again see that the
angular momentum about point P has not changed:
\begin{equation*}
  L_3=r_3mv\sin\phi
  =\left[\frac{r_2}{\cancel{\sin\phi}}\right]mv\cancel{\sin\phi}
  =(r_1\sin\theta)mv=L_1
\end{equation*}
An object is uniform motion is not subjected to any net force, therefore there
cannot be a net torque on the object about \emph{any} point. We must conclude
that both translational momentum ($\bm p=m\bm v$) and angular momentum
($\bm L=\bm r\times\bm p$) are constant in uniform motion.


\begin{example}
  A \SI{1.0}{\kilo\gram} mass swings in a vertical circle
...
\section{Conservation of Angular Momentum in a Collision}

In a collision between two objects, not only is translational momentum
conserved, angular momentum must be conserved as well.
...
As we have seen in Chapter~\ref{chapter:energy}, for translational motion,
mechanical work in a one-dimensional coordinate system is defined as:
\begin{equation*}
  W_t=\int_{x_0}^{x_1}F(x)\dl x
\end{equation*}
If the resulting motion is a rotation, then we can rewrite the object's
displacement as $\dl x=r\dl\theta$. Then we can calculate the rotational work
done by the force:
\begin{equation}
  W_r=\int_{x_0}^{x_1} F\dl x
  =\int_{\theta_0}^{\theta_1} \underbracket[1pt]{F (r}_\tau\dl\theta)
\end{equation}
Noting that $\tau=rF$, rotational work becomes:
\begin{important-equation}
  W_r=\int_{\theta_0}^{\theta_1}\tau(\theta)\dl\theta
\end{important-equation}
Just as we did with translational work, we must distinguish between
\begin{itemize}
\item\textbf{rotational work done by a specific torque} acting on the object,
  and
\item\textbf{the total rotational work done on an object},
\end{itemize}
...
\begin{important-equation}
  \text{Work-energy principle:}\quad W_{r,\text{net}}=\Delta K_r
\end{important-equation}
Just as the net/total translational work on an object changes its translational
kinetic energy (Eq.~\ref{work-energy-theorem}),
%in Chapter~\ref{chapter:energy}),
...
its centre of mass (Fig.~\ref{disk1}). To analyse its rotational motion, we
first find its moment of inertia about the pivot from Fig.~\ref{moment-3d}. For
a uniform disk, $I=\frac12MR^2$. Since the centre of mass is also the pivot, we
only have to consider its rotational motion.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
    \draw[mass] circle (2);
