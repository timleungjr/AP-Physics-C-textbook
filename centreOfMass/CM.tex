...
\subsection{Slightly More Challenging}
What if one of the masses are increased to $2m$? This is still not a difficult
problem; you can still \emph{guess} the right answer without knowing the
equation for centre of mass. 
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
...
\subsection{Many Point Masses}
The weighted average concept can now be applied to cases when there are masses
...
\end{equation}
The centre of mass would be a function of time, i.e.\ $\bm r=\bm r(t)$ if any
of the masses are in motion. We can break $\bm r$ into its components:
\begin{equation*}
  x_\text{cm}(t)=\frac{\sum x_i(t) m_i}{\sum m_i}\quad\quad
  y_\text{cm}(t)=\frac{\sum y_i(t) m_i}{\sum m_i}\quad\quad
  z_\text{cm}(t)=\frac{\sum z_i(t) m_i}{\sum m_i}
\end{equation*}
...
\begin{equation}
  \boxed{\bm x_\text{cm}=\frac{\int\bm x\dl m}{\int\dl m}}
\end{equation}
...
\begin{figure}[ht]
  ...
  \end{subfigure}
  \caption{Centroids of some 2D shapes}
  \label{tabl:centroids}
\end{figure}
...
%\begin{table}[ht]
%  \centering
%  \pic{.8}{centreOfMass/eng130C9_11}
%  \caption{Centroids of common two-dimensional shapes.}
%  
%\end{table}
...
\begin{equation}
  \bm v_\text{cm}=\diff{\bm x_\text{cm}}t
  =\frac1m\diff{}t\left(\int\bm x\dl m\right)
  =\frac1m\int\diff{\bm x}t\dl m
  =\frac{\int\bm v\dl m}m
\end{equation}
Or, in the form that is familiar, the velocity of the CM is the weighted sum of
the velocities of the distribution of mass:
\begin{equation}
  \bm v_\text{cm} = \frac{\int\bm v\dl m}m
\end{equation}
We can also rearrange the equation for the velocity of the centre of mass to
relate it to momentum, because the term $\int\bm v\dl m$ is the net momentum
