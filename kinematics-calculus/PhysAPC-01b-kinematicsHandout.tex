...
\begin{equation*}
  \mathbf r(t)=
  \underbrace{x(t)\iii}_{\mathbf x(t)} +
  \underbrace{y(t)\jjj}_{\mathbf y(t)} +
  \underbrace{z(t)\kkk}_{\mathbf z(t)}
\end{equation*}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.6]
    ...
    \draw[vector,red!80!black] (0,0)--(4,3) node[midway,above]{$\mathbf r(t)$};
    \draw[vector,orange] (0,0)--(0,3) node[midway,left]{$\mathbf y(t)$};
    \draw[vector,violet] (0,0)--(4,0) node[midway,below]{$\mathbf x(t)$};
    %    \draw[thick,dash dot,<->] (4,1)..controls (6,5) and (5,7)..(2,6)
    %    node[midway,right]{$s$};
  \end{tikzpicture}
  \caption{Position vector of an object inside a 2D cartesian coordinate
    system. The vectors $\mathbf x$ and $\mathbf y$ are the $x$ and $y$ components of
    $\mathbf r$ respestively.}
...
the initial position ($\mathbf r_o$) to the current position ($\mathbf r(t)$) within
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.6]
    \draw[axes] (0,0)--(5.5,0) node[right]{$x$};
    \draw[axes] (0,0)--(0,4.5) node[above]{$y$};
    \draw[vector,red!80!black] (0,0)--(4,1) node[midway,below]{$\mathbf r_i$};
    \draw[vector,red!80!black] (0,0)--(1,3.5) node[midway,left]{$\mathbf r(t)$};
    \draw[vector,blue] (4,1)--(1,3.5) node[midway,right]{$\Delta \mathbf r(t)$};
  \end{tikzpicture}
...
\begin{equation}
  \Delta\mathbf r(t)=%\Delta\mathbf r_x(t)-\mathbf r_i
...
$\mathbf r_\text{ref}=\mathbf0$, any position vector $\mathbf r$ is also its displacement
...
from position $\mathbf r_o$ to current position $\mathbf r(t)$, as shown in
...
moving. If position $\mathbf r(t)$ is differentiable in time $t$, then its
\textbf{instantaneous velocity} $\mathbf v(t)$ can be found at any time $t$ by
differentiating $\mathbf r$ with respect to $t$. The SI unit for velocity is
\emph{meters per second} (\si{\metre\per\second}):
\begin{equation}
  \boxed{\mathbf v(t)= \diff{\mathbf r(t)}t}
\end{equation}
Since position $\mathbf r(t)$ has $x(t)$, $y(t)$ and $z(t)$ components along the
...
\begin{equation*}
  \mathbf v(t)= \diff{\mathbf r}t=
  \diff xt\iii + \diff yt\jjj + \diff zy\kkk =
  v_x\iii + v_y\jjj + v_z\kkk
\end{equation*}
By the fundamental theorem of calculus, if instantaneous velocity $\mathbf v(t)$
is the time derivative of position $\mathbf r(t)$ with respect to time $t$, then
$\mathbf r(t)$ is the time integral of $\mathbf v(t)$:
\begin{equation}
  \boxed{\mathbf r(t)=\int\mathbf v(t)\dl t + \mathbf r_0}
  \quad\text{or}\quad
  \boxed{\Delta\mathbf r(t)=\int_{t_0}^t\mathbf v(t)\dl t}
\end{equation}
The constant of integration $\mathbf r_0=\mathbf r(0)$ is the object's \emph{initial
position} at $t=0$. As was the case in differentiation, we can integrate each
component to get $\mathbf r$:
\begin{equation*}
  \mathbf r(t)= \left(
  \int v_x(t)\iii + \int v_y(t)\jjj + \int v_z(t)\kkk \right)\dl t + \mathbf r_0
\end{equation*}


The \textbf{average velocity} ($\overline{\mathbf v}$)\footnote{For
...
$\Delta\mathbf r$ over a finite time interval $\Delta t$:
\begin{equation}
  \boxed{
    \overline{\mathbf v}(t)= \frac{\Delta\mathbf r}{\Delta t}
      =\frac{\int_{t_0}^t \mathbf r(t)dt}{t-t_0}
  }
\end{equation}
...
velocity vector $\mathbf v$. Likewise, \textbf{average speed} ($\overline v(t)$) is
...
to time, \textbf{instantaneous acceleration} $\mathbf a(t)$ is the rate of change
...
\begin{equation}
  \boxed{\mathbf a(t)= \diff{\mathbf v(t)}t=\diff[2]{\mathbf r(t)}t}
\end{equation}
...
%cases.
%  \begin{enumerate}
%  \item Take derivative of $\mathbf x(t)$ to get $\mathbf v(t)=\mathbf x'(t)$
%  \item Take derivative again of $\mathbf v(t)$ to get $\mathbf a(t)=\mathbf v'(t)$
%  \end{enumerate}
%\end{frame}
Again, by the fundamental theorem of calculus, instantaneous velocity
$\mathbf v(t)$ is the time integral of instantaneous acceleration $\mathbf a(t)$:
\begin{equation}
  \boxed{\mathbf v(t)=\int\mathbf a(t)\dl t+\mathbf v_0} =
  \left(
  \int a_x\iii +
  \int a_y\jjj +
  \int a_z\kkk \right)\dl t +\mathbf v_0
\end{equation}
where $\mathbf v_0$ is the initial velocity at $t=0$.
...
\item\textbf{Spring force} is proportional to displacement\footnote{By Hookes's
  law $\mathbf F_s=-k\mathbf x$, where $k$ is the spring constant that describes the
...
of position, i.e.\ $\mathbf v=\dot{\mathbf r}$ while acceleration is the second
derivative, i.e.\ $\mathbf a = \dot{\mathbf v}=\ddot{\mathbf r}$. This notation will be
...
acceleration is called \textbf{jerk} $\mathbf j(t)$ with a unit of
\si{\metre\per\second\cubed}:
\begin{equation}
  \mathbf j(t)=\diff{\mathbf a}t=\diff[2]{\mathbf v}t=\diff[3]{\mathbf r}t
\end{equation}
...
\begin{equation}
  \mathbf s(t)=\diff{\mathbf j}t=\diff[2]{\mathbf a}t=\diff[3]{\mathbf v}t=\diff[4]{\mathbf r}t
\end{equation}
...
$\mathbf a$, velocity can be obtained by integrating in time:
\begin{equation}
  \mathbf v(t)=\int\mathbf a\dl t\quad\rightarrow\quad
  \boxed{\mathbf v(t)=\mathbf v_0+\mathbf at}
  \label{eq:big5-1}
\end{equation}
where $\mathbf v_0$ is the initial velocity at $t=0$. Integrating again for the
position vector:
\begin{equation}
  \mathbf r(t)=\int \mathbf v\dl t\quad\rightarrow\quad
  \boxed{\mathbf r(t)=\mathbf r_0+\mathbf v_0t+\frac12 \mathbf at^2}
  \label{eq:big5-2}
\end{equation}
where $\mathbf r_0$ is the initial position at $t=0$. The position vector is
quadratic in time.
...
%  \begin{itemize}
%  \item Time interval $\Delta t$ and displacement $\Delta\mathbf x$ of the two
%    objects are related
%  \item Examples:
%    \begin{itemize}
