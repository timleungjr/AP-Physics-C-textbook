...
\textbf{instantaneous velocity} $\bm v(t)$ at any time $t$ can be found at any
time $t$ by differentiating $\bm r$ with respect to $t$. The SI unit for
velocity is \emph{metres per second} (\si{\metre\per\second}):
\begin{important-equation}
  \bm v(t)= \diff{\bm r(t)}t
\end{important-equation}
Since position $\bm r(t)$ has $x(t)$, $y(t)$ and $z(t)$ components along the
(linearly independent) $\iii$, $\jjj$ and $\kkk$ directions, we can
independently take the time derivative in every direction to obtain the
...
is the time derivative of position $\bm r(t)$, then $\bm r(t)$ and displacement
$\Delta\bm r(t)$ are both time integrals of $\bm v(t)$:
\begin{important-equation}
  \bm r(t)=\int\bm v(t)\dl t + \mathcal R
  \quad\text{and}\quad
  \Delta\bm r(t)=\int_{t_0}^t\bm v(t)\dl t
\end{important-equation}
The difference between integrating $\bm v$ to get $\bm r$ or $\Delta\bm r$ is
that the integral for position is indefinite, while the integral for
displacement is definite. The constant of integration $\mathcal R$ is based on
the object's initial position at $t=0$. As was the case in differentiation,
we can integrate each component to get $\bm r$:
\begin{equation*}
  \bm r(t)= \left(
  \int v_x(t)\iii +
  \int v_y(t)\jjj +
  \int v_z(t)\kkk \right)\dl t +
  \mathcal R
\end{equation*}
\begin{remark}
  In my years of teaching, there have been many students who assume that
  $\mathcal R$ \emph{is} the initial position. However, while this may be true
  if $\bm r(t)$ is a polynomial function of time, this is not true in all cases.
\end{remark}

The \textbf{average velocity} ($\overline{\bm v}$)
%\footnote{For
...
of an object is the displacement $\Delta\bm r$ over a finite time interval
$\Delta t$:
\begin{important-equation}
  \overline{\bm v}(t)= \frac{\Delta\bm r(t)}{\Delta t}
  =\frac{\int_{t_0}^t \bm r(t)dt}{t-t_0}
\end{important-equation}
Since displacement is a function of time, average velocity is also a function
of time. Like instantaneous velocity, we can find the $x$, $y$ and $z$
components of average velocity by separating components in each direction:
\begin{equation*}
  \overline{\bm v}=
...
respect to time.
%\footnote{It is regrettable that both velocity and speed use
%  the symbol $v$, but \emph{c'est la vie}.}
Like velocity, the unit for speed is also \si{\metre\per\second}:
\begin{important-equation}
  v(t)=\diff st\geq 0
\end{important-equation}
Since distance is a scalar quantity, so too is speed. As distance along any
path must be non-negative and increasing in time as an object moves, so must
the instantaneous speed, i.e.\ $v(t)\geq 0$. Instantaneous speed $v$ is the
magnitude of the instantaneous velocity vector $\bm v$. (This is the reason
that both speed and velocity use the symbol $v$.)

Likewise, \textbf{average speed} ($\overline v(t)$) is similar to average
velocity: it is the distance travelled over a finite time interval.\footnote{It
should be obvious that unlike instantaneous speed, average
speed is not the magnitude of the average velocity.}
\begin{important-equation}
  \overline v(t)=\frac{s(t)}{\Delta t}
\end{important-equation}
...
The SI unit for acceleration is \emph{metres per second squared}
(\si{\metre\per\second\squared}):
\begin{important-equation}
  \bm a(t)= \diff{\bm v(t)}t=\diff[2]{\bm r(t)}t
\end{important-equation}
...
%  \int a_x\iii +
%  \int a_y\jjj +
%  \int a_z\kkk \right)\dl t +\bm v_0
\end{important-equation}
where $\bm v_0$ is the constant of integration based on initial velocity at
$t=0$ (it does \emph{not} mean that $\bm v_0$ is the initial velocity!)
...
often expressed as functions of other motion quantities rather than time.

\textbf{Gravitational force}\footnote{Newton's law of universal
gravitation: $F_g=\dfrac{Gm_1m_2}{r^2}$} or \textbf{electrostatic
  force}\footnote{Coulomb's law: $F_q=\dfrac{kq_1q_2}{r^2}$} are both inversely
proportional to the square of the distance (called the inverse-square law), and
therefore acceleration is best express by this complicated differential
equation:
\begin{equation*}
...
\textbf{Spring force} is proportional to displacement\footnote{By Hookes's law
$\bm F_s=-k\bm x$, where $k$ is the spring constant that describes the
...
motion} that will be studied in Chapter~\ref{chapter:harmonic-motion}.

\textbf{Viscous damping force}, like those exerted by a shock absorber, is
usually proportional to velocity, leading to an expression for acceleration:
\begin{equation*}
...
integrating, and the expression for velocity is an exponential decay function:
\begin{equation*}
...
\textbf{Aerodynamic forces} such as drag and lift\footnote{The equations
...
\begin{equation*}
  \bm v=\dot{\bm r}
\end{equation*}
while acceleration is the second derivative of position, and the first
derivative of velocity
\begin{equation*}
  \bm a = \dot{\bm v}=\ddot{\bm r}
\end{equation*}.
This notation will be used when it is convenient to do so.


\subsection{Higher Derivatives of Position}

For anyone curious about higher derivatives, the time derivative of
acceleration is called \textbf{jerk} $\bm j(t)$, with an SI unit of
\emph{metres per second cubed} (\si{\metre\per\second\cubed}):
\begin{equation}
  \bm j(t)=\diff{\bm a}t=\diff[2]{\bm v}t=\diff[3]{\bm r}t
\end{equation}
The measurement of jerk is meaningful in many sensors, for example, in
...
The time derivative of jerk is \textbf{jounce}, or \textbf{snap}, with unit of
...
but these higher derivatives have little to no physical meaning, and are
...
Although kinematic problems in often require calculus,
%\footnote{Unlike
%your AP Calculus exams, the differentiation/integration in AP Physics will be
%fairly straightforward},
basic kinematic equations for \emph{constant} acceleration are still a very
powerful tool. For constant acceleration $\bm a$, velocity can be obtained by
integrating in time:
\begin{equation}
