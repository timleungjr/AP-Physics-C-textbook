...
kinetic energy in $m_2$ as it moves from $x_0\to x_1$ is
\begin{equation*}
...
Without static friction, the net force on $m_1$ would have just been $\bm F$,
and the change in kinetic energy moving from $x_0\to x_1$ to the right is simply
\begin{equation*}
...
\textbf{absolute temperature}, measured in \emph{kelvin}. Discussions on
thermal/internal energy and the behaviour of gases and solids
are part of a much larger discipline within physics called
\textbf{thermodynamics}, but it is outside the scope of this book. %chapter.
\begin{remark}
  For those who are keen to know, for a monatomic ideal gas, the internal
  energy comes entirely from the kinetic energy from the translational motion
  of the molecules, %from the 3 degrees to translational freedom, and
  given by:
  \begin{equation*}
  ...
  %rotational freedom, and the
  internal energy is given by:
  \begin{equation*}
    E_\text{int}\approx\dfrac52Nk_bT
  \end{equation*}
  ...
  degrees of vibrational freedom, and therefore the
  internal energy is:
  \begin{equation*}
...
\chapter{Law of Conservation of Energy}
...
The only force that acts on the mass is the gravitational force $\bm F_g$,
...
forces, this is an isolated system. As the object falls, the gravitational force
...
Since gravity is a conservative force, the positive work done transforms
gravitational potential energy into kinetic energy of the mass by the same
amount, and since $\bm F_g$ is an internal force, the change in the system's
energy is zero.
\begin{equation}
  \Delta K + \Delta U_g=0
\end{equation}
...
\begin{example}
  ...
  third-floor balcony of an apartment building, at a height $h$ above the
  ground. If friction and drag can be ignored, with what speed $v'$ would it
  hit the ground?
  
  \textbf{Solution:} This is a simple problem that any beginner physics student
  can solve using kinematics, but we will solve it using conservation of
  energy instead. Since there is no friction and drag, the total mechanical
  energy is conserved between when the ball is thrown, and the moment just
  before it hits the ground. Setting $h=0$ at ground level, the gravitational
  potential is zero ($U_g=mgh=0$) just before the ball hits the ground:
  \begin{displaymath}
  ...
  should expect!), but with some subtle differences:
  \begin{itemize}[leftmargin=10pt]
  \item This is a scalar problem, so we didn't bother to define a positive
    direction, we we would have to do when solving a vector problem. We
    actually don't know explicitly the direction of the ball when it hits the
    ground (although it is plainly obvious that it falls downwards).
  \item We cannot find out how long it took to hit the ground.
  \item All we know is the speeds at the end points.
  \end{itemize}
...
%TL%            \draw[thick] (-.2,2) rectangle (.2,2.4);
%TL%            \draw[vectors] (0,2.2)--(.8,2.2) node[right]{$\mb{v}$};
%TL%          \end{scope}
...
%TL\newpage
%TL
%TL%\genfreetitle{3}{WORK AND ENERGY}{3}
%TL%\genfreedirections
