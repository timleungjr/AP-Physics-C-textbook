...
by $\mathbf F$ can be positive or negative depending on the dot product. In
...
$\mathbf F\cdot\dl\mathbf x=0$) which means that the force did not \emph{cause}
the displacement, or if the object does not move, (i.e.\
$\dl\mathbf x=\mathbf 0$), or if no force is applied during motion (i.e.\
$\mathbf F=\mathbf 0$).
...
also use the above equation if the force $\mathbf F$ is \emph{averaged} over
the displacement, i.e.\
\begin{equation}
  \mathbf F=\mathbf F_\text{avg}=\frac{\int_{x_0}^{x_1} F(x)\dl x}{x_1-x_0}
\end{equation}
\begin{sidenote}
  ...
  \begin{displaymath}
    \overline{\mathbf F}=\frac{\int_{t_0}^{t_1} \mathbf F(t)\dl t}{t_1-t_0}
  \end{displaymath}
  ...
  of a single object:
  \begin{displaymath}
    \overline{\mathbf v}=\frac{\int_{t_0}^{t_1} \mathbf v(t)\dl t}{t_1-t_0}
    \quad\text{and}\quad
    \overline{\mathbf a}=\frac{\int_{t_0}^{t_1} \mathbf a(t)\dl t}{t_1-t_0}
  \end{displaymath}
...
At this moment, it is unclear what the force $\mathbf F$ is. Whereas we use the
...
\item\textbf{Work done by the \emph{net} force}, in other words, the \emph{sum}
  ...
  \begin{equation*}
    W_\text{net}
    = \sum_i W_i
    =\int_{\mathcal C}\mathbf F_\text{net}(\mathbf x)\cdot\dl\mathbf x
  \end{equation*}
...
\begin{example}
  ...
  the crate: gravity $\mathbf F_g$, normal force $\mathbf F_n$, kinetic
  friction $\mathbf f$, and applied force $\mathbf F_a(x)$, which is expressed
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \begin{scope}[rotate=25]
        ...
        \draw[vector] (2.5,.75)--+(0,1.5) node[above]{$\mathbf F_n$};
        \draw[vector,rotate around={-25:(2.5,.75)}]
        (2.5,.75)--+(0,-1.25) node[below]{$\mathbf F_g$};
        \draw[vector] (2.5,.75)--+(-1,0) node[left]{$\mathbf f$};
      \end{scope}
    \end{tikzpicture}
  \end{center}
  We can calculate the work done by each force:
  \begin{itemize}[leftmargin=12pt]
  \item Work done by the normal force is zero, because $\mathbf F_n$ is
    perpendicular to the direction of motion:
    \begin{equation*}
      W_n=\int\underbrace{\mathbf F_n\cdot\dl\mathbf x}_{=0}=0
    \end{equation*}
  \item Work done by kinetic friction is negative, because $\mathbf f$ is in
    the opposite direction to motion:
    \begin{equation*}
      W_f=\int\mathbf f\cdot\dl\mathbf x=-\int f\dl x=-\mu_k F_n\Delta x<0
    \end{equation*}
  ...
  \begin{align*}
    ...
    &=\int\left(\mathbf F_a+\mathbf F_n+\mathbf F_g+\mathbf F_f\right)
    \cdot\dl\mathbf x =\int\mathbf F_\text{net}\cdot\dl\mathbf x
  \end{align*}
\end{example}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.6]
    ...
    \draw[vector,red] (0,6)--+(0,-2) node[right]{$\mathbf F_g$};
...
When displacement $\Delta\mathbf x$ is small, acceleration due to gravity
$\mathbf g$ can be considered to be constant, therefore
$\mathbf F_g=m\mathbf g$ is a constant force. Work done by gravity can be
calculated using Eq.~\ref{eq:no-integration}, and no integration is needed.
Since both $\mathbf F_g$ and $\Delta\mathbf x$ are in the same direction, work
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.65]
    \draw[vector,red] (0,0)--(2,0) node[right]{$\mathbf F_g$};
    \draw[vector,blue] (8,0)--(6,0) node[left]{$\mathbf F_g$};
...
\begin{equation}
  W_g = \int_{\mathbf r_1}^{\mathbf r_2}\mathbf F_g\cdot\dl\mathbf r
...
The \textbf{spring force}, or \textbf{elastic force} $\mathbf F_e$ is the force
...
displacement $\mathbf x$, and acts in the opposition to the displacement:
\begin{equation}
  \boxed{
    \mathbf F_e=-k\mathbf x
  }
\end{equation}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \draw[vector,red] (5.5,1)--(4,1) node[above]{$\mathbf F_e$};
  ...
  \begin{tikzpicture}
    ...
    \draw[vector,red] (2,1)--(3,1) node[above]{$\mathbf F_e$};
  \end{tikzpicture}
...
\fcolorbox{black}{yellow!10}{
  \begin{minipage}{.97\textwidth}
    ...
    \item Work by the spring force is \emph{path independent}: $W_e$ depends on
      ...
      $x_0$ to $x_1$
    \item Only work done by $\mathbf F_e$ can affect $U_e$
    \end{itemize}
  \end{minipage}
}
...
to calculate the maximum acceleration $\mathbf a_\text{max}$ of the blocks
without them sliding against each other, and the maximum applied force
$\mathbf F_\text{max}$ associated with that maximum acceleration.}

Two blocks with masses $m_1$ and $m_2$, stacked vertically, move to the right
without slipping by an external applied force $\mathbf F$ applied to $m_1$, as
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    \fill circle (.1);
    \draw[vector] (0,0)--(0,-1) node[below]{$\mathbf F_{g2}$};
    \draw[vector] (0,0)--(0, 1) node[above,fill=pink!30]{$\mathbf N_{12}$};
    \draw[vector] (0,0)--(1.5,0) node[right,fill=yellow!30]{$\mathbf f$};

    \fill (5,0) circle (.1);
    \draw[vector] (4.97,0)--+(0,-1) node[below left]{$\mathbf F_{g1}$};
    \draw[vector] (5.03,0)--+(0,-1)
    node[below right,fill=pink!30]{$\mathbf N_{12}$};
    \draw[vector] (5,0)--+(0,1) node[above]{$\mathbf N_1$};
    \draw[vector] (5,0)--+(-1.5,0) node[left,fill=yellow!30]{$\mathbf f$};
    \draw[vector] (5,0)--+(2,0) node[right]{$\mathbf F$};
  \end{tikzpicture}
...
friction $\mathbf f$ at the interface with $m_1$. (Without friction, $m_2$
...
$\mathbf N_{12}$ and gravitational force $\mathbf F_{g2}$ are perpendicular to
motion, and therefore, do not do any work.
%, and the work done by $\mathbf f$ is
%\emph{positive}.
...
\end{equation*}
Note that since we are not assuming whether applied force $\mathbf F$ is
...
does positive work, while in this case, static friction $\mathbf f$ does
...
%between $m_1$ and $m_2$,
the net force on $m_1$ would have just been $\mathbf F$, and the change in
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.75]
    ...
    \draw[vector] (.5,2.8)--(.5,2.1) node[midway,left]{$\mathbf x$};
    \draw[fill=gray!10] (-.1,-.1) rectangle (2.1,2.1);
    \draw[vector] (2.5,2)--(2.5,0) node[midway,right]{$\mathbf v$};
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.65]
    ...
    \draw[vector,red] (0,7)--+(0,-2) node[below=-2]{$\mathbf F_g$};
    \draw[vector,red] (0,.5)--+(0,2) node[above=-2]{$\mathbf F_g$};
  \end{tikzpicture}
...
The only force that acts on the mass is the gravitational force $\mathbf F_g$,
...
amount, and since $\mathbf F_g$ is an internal force, the change in the system's
energy is zero.
\begin{equation}
  \Delta K + \Delta U_g=0
\end{equation}
...
\begin{example}
  \textbf{Question:} A ball is thrown upwards with an initial speed of
  \SI5{\metre\per\second} from the third-floor balcony of an apartment
  building, \SI{10}{\metre} above the ground. If friction and air resistance
  can be ignored, with what speed would it hit the ground?
  
  \vspace{.1in}\textbf{Solution:} This is a simple problem that any beginner
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \draw[vector,red] (1.4,2.8)--+(0,-1.5) node[left]{$\mathbf F_g$};
    \draw[vector,red,rotate around={30:(1.4,2.8)}] (1.4,2.8)--+(1.4,0)
    node[right]{$\mathbf F_n$};
  \end{tikzpicture}
  \caption{A mass sliding along an arbitrary ramp is an isolated system.}
  \label{fig:ramp-system}
\end{figure}
Normal force $\mathbf F_n$ and gravity $\mathbf F_g$ act on the object, but
only gravity does work ($\mathbf F_n$ is always perpendicular to motion). The
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \draw[vector,red] (5.5,1)--(5.5,0) node[below]{$\mathbf F_g$};
    \draw[vector,red] (5.5,1)--(5.5,2) node[above]{$\mathbf F_n$};
    \draw[vector,red] (5.5,1)--(4.5,1) node[above]{$\mathbf F_e$};
    \fill[red] (5.5,1) circle (.06);
  \end{tikzpicture}
...
forces act on the system: the gravitational force ($\mathbf F_g$) and normal
force ($\mathbf F_n$) which act on the mass. However, these forces do not do
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \draw[vector,red] (1,2)--(1,1) node[right]{$\mathbf F_g$};
    \draw[vector,red] (1,2)--(1,3) node[right]{$\mathbf F_e$};
...
forces in the spring, the only forces are the spring force $\mathbf F_e$ and
the gravitational force $\mathbf F_g$, both of which are \emph{internal} to the
...
%      does work
%    \item Tension ($\mathbf F_T$), which is non-conservative, does not do work on the
...
$\mathbf F_T$, and the gravitational force $\mathbf F_g$. Gravitational force
is an internal force (because it acts on both the pendulum bob and Earth), but
tension force is an \emph{external} force. However, since $\mathbf F_T$ is
