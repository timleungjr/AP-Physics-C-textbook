...
\end{equation}
\begin{remark}
  At this time, we need a short discussion about what this \emph{average} is.
  In this case, the force is averaged over the \emph{displacement} that the
  object travels. This is called a \emph{spatial average}. We are not concerned
  about how long it takes for the object to move. In contrast, when computing
  the \emph{impulse} generated from an average force over a \emph{time}
  interval, which is related to the change in the \emph{momentum} of an object,
  the force is averaged over \emph{time}. This is called a \emph{time average}:
  \begin{displaymath}
    \overline{\bm F}=\frac{\int_{t_0}^{t_1} \bm F(t)\dl t}{t_1-t_0}
  \end{displaymath}
  Here, we purposely use different notation to distinguish different types of
  averaging. Time averaging does not consider the actual motion of the object.
  This time averaging is also how we get the expressions for average velocity
  and average acceleration in Chapter~\ref{chapter:kinematics}, when we
  describe the motion of a single object:
  \begin{displaymath}
  ...
  which uses a time averaging.

  However, there is still a third kind of averaging that we will briefly touch
  on in this chapter. In the thermodynamics of gases, when we calculate the
  average speed and kinetic energies of many gas molecules, the average is the
  arithmetic mean of all the speeds:
  \begin{displaymath}
...
\begin{equation*}
  ...
  =\Delta K
\end{equation*}
where $K$ is defined as the \textbf{kinetic energy}:
\begin{important-equation}
  K=\frac12mv^2
  \label{eq:KE-definition}
\end{important-equation}
\begin{remark}
  $K$ is more specifically called the \textbf{translational kinetic energy},
  and must be distinguished from the \textbf{rotational kinetic energy} which
  is used when an object is rotating about a pivot. The topic of rotational
  motion and rotational kinetic energy is discussed in detail in
  Chapter~\ref{chapter:rotMotion}.
\end{remark}
...
This is known as the \textbf{work-energy principle}:
%\footnote{Also known as the
...
\emph{increase} the kinetic energy of the object by the same amount, while
\emph{negative} net work will decreases kinetic energy by the same amount.
Eq.~\ref{work-energy-theorem} applies regardless of \emph{what} the net force
is comprised of.

\textbf{Crucially, the work-energy principle turns a generally difficult vector
integration problem (integrating $W_\text{net}$ on the left hand side) into a
simple scalar algebraic expression ($\Delta K$ on the right hand side). As we
will see later in the chapter, this change from a vector calculus problem to a
scalar algebra problem has profound implications in our ability to solve
motion problems quickly.}
\begin{remark}
  When you integrate the net work done ``properly'' in two or three dimensions,
  the only difference is the dot product, which only \emph{slightly} increases
  the complexity of the problem compared to the 1D case:
  \begin{equation*}
    W_\text{net}
    = \int\bm F_\text{net}(\bm x)\cdot\dl{\bm x}
    = \cdots = m\int_{\bm v_0}^{\bm v_1}\bm v\cdot\dl{\bm v}
  \end{equation*}
  The dot product is a lot easier to evaluate than you might think (albeit it
  makes our algebra more tedious)
  \begin{equation*}
    \bm v\cdot\dl{\bm v}=v_x\dl v_x+v_y\dl v_y+v_z\dl v_z
  \end{equation*}
  In this case, we have to add up the work done in all three dimensions of
  motion:
  \begin{align*}
    m\int\bm v\cdot\dl{\bm v}
    &=m\left(
    \int_{v_{x0}}^{v_{x1}} v_x\dl v_x +
    \int_{v_{y0}}^{v_{y1}} v_y\dl v_y +
    \int_{v_{z0}}^{v_{z1}} v_z\dl v_z\right)\\
    &=\left(\frac12mv_{x1}^2+\frac12mv_{y1}^2 + \frac12mv_{z1}^2\right)-
    \left(\frac12mv_{x0}^2+\frac12mv_{y0}^2 + \frac12mv_{z0}^2\right)
  \end{align*}
  Noting that $v^2=v_x^2+v_y^2+v_z^2$, the result of this integrating is the same
  as in the one-dimensional case:
  \begin{displaymath}
    W_\text{net}=\frac12mv_1^2-\frac12mv_0^2=\Delta K
  \end{displaymath}
\end{remark}
Even though we are able to equate the net work done on an object to the change
in kinetic energy in 1D, 2D and 3D cases, it should be noted that
\begin{itemize}
\item In a one-dimensional coordinate system, the net force on an object
  would always be either along the direction of motion, or opposite to
  motion. Therefore net force would \emph{always} do work, and there would
  always be a change in energy.
\item In two or higher dimensions, the net force can be perpendicular to
  motion. Therefore the presence of a net force does \emph{not} mean that the
  net force is doing work. A good example is in circular motion
  (Chapter~\ref{chapter:circ-motion}), where the net force towards the centre of
  the circular path, called the centripetal force, can never do any work.
\end{itemize}
...
\begin{example}
  ...
  \textbf{Solution:} Will be available when I am ready to write it out.
\end{example}
...
Consider an object that is moving (free-falling) under the force of gravity,
%near the surface of Earth,
over a distance of $\Delta x$, as shown in Fig.~\ref{fig:falling1}. We have
chosen a free fall because our goal is to isolate the work done by
gravitational force alone, without worrying about any other forces.
\begin{figure}[ht]
  \centering
...
calculated using Eq.~\ref{eq:no-integration}, and no integration is needed. If
the object falls downwards, i.e.\ both $\bm F_g$ and $\Delta\bm x$ are
downwards, work done by gravitational force ($W_g$) is positive. Since
gravitational force \emph{is} the net force, work by gravity is the net work
($W_g=W_\text{net}$). From the work-energy principle
(Eq.~\ref{work-energy-theorem}), there is an increase in kinetic energy
\emph{by the same amount as work}, and the object speeds up:
\begin{equation*}
  W_g=W_\text{net}=mg\Delta x>0 \quad\longrightarrow\quad \Delta K = W_g > 0
\end{equation*}
Conversely, if the object moves upwards, i.e.\ $\bm F_g$ and $\Delta\bm x$ are
in the opposite direction, then work done by gravitational force is negative.
There would now be a decrease in kinetic energy
\emph{by the same amount as work}, and the object slows down.
\begin{equation*}
  W_g=W_\text{net}=mg\Delta x<0 \quad\longrightarrow\quad \Delta K = W_g < 0
\end{equation*}
...
in height. Using the ground as the reference level (i.e.\ $h=0$), we can
express the displacement $\Delta x$ in terms of the initial height $h_0$ and
final height $h_1$. The work done by gravity can therefore be written as:
\begin{equation*}
  W_g = mg(h_0-h_1)
\end{equation*}
...
\begin{definition}
  \begin{itemize}
  \item When work done by gravity \emph{positive}, (i.e.\ $W_g>0$), there is a
    \emph{decrease} in gravitational potential energy (i.e.\ $U_g<0$) by the
    same amount.
  \item When work done by gravity is \emph{negative}, (i.e.\ $W_g<0$), there is
    a \emph{increase} in gravitational potential energy (i.e.\ $U_g>0$) by the
    same amount
  \item Positive work by gravity transforms gravitational potential energy into
    kinetic energy; negative work by gravity transforms kinetic energy into
    gravitational potential energy
  \item Work by the gravitational force is \emph{path independent}: $W_g$
    depends on the end points $h_0$ and $h_1$, but not \emph{how} the object
    moves from $h_0$ to $h_1$
  \item Only work done by gravitational force $\bm F_g$ can affect
    gravitational potential energy $U_g$
  \end{itemize}
\end{definition}

If the displacement $\Delta\bm x$ is sufficiently large that we can no longer
treat $\bm g$ as a constant, we will have to use the law of universal
gravitation:
\begin{equation*}
  F_g=\frac{Gm_1m_2}{r^2}
\end{equation*}
In this case, the general expression for gravitational potential energy can be
obtained by integrating the law of universal gravitation, giving us the
equation:
\begin{equation*}
  U_g=-\frac{Gm_1m_2}r
\end{equation*}
The topic of gravity is discussed further in Chapter~\ref{chapter:gravity}.




\subsubsection{Path Independence}
As we can see in Fig.~\ref{path-independence}, the work done by gravity is the
same in all cases. That is not to say that there are no other forces acting on
the object; we have merely isolated the work done by the gravitational force
alone.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
  ...
  \end{tikzpicture}  
  \caption{Work done by gravity is the same in all above cases because
...
Before we leave the topic of gravitational potential energy, there are a few
more details that we probably want to address.
\begin{remark}
  By the third law of motion, just as an object is free falling under the
  gravitational force from Earth, the object also exerts an equal and opposite
  reaction force on Earth. However, since the mass of Earth is much higher,
  this minuscule force would not cause any measurable acceleration, therefore
  there is nearly no work done on Earth. Action and reaction forces may be
  equal and opposite, but the work that they do are rarely the same.
\end{remark}
...
\begin{equation*}
  \bm F_e=-k\bm x
\end{equation*}
The constant $k$ that relates the spring force and spring displacement is
called the \textbf{spring constant}\footnote{The spring constant is also called
the \textbf{force constant}, \textbf{Hooke's constant}, and in many engineering
textbooks and journals, \textbf{spring rate}.}, with a unit of
\emph{newton per metre} (\si{\newton\per\metre}). The spring constant
represents the stiffness of the spring, and it depends on the size and geometry
of the spring, as well as the material that it is made of.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
  ...
  \end{tikzpicture}
  \caption{Direction of spring force and direction of spring displacement.}
  \label{hooke1}
\end{figure}
...
Sec.~\ref{section:gravitationalPE}:
\begin{definition}
  \begin{itemize}
  ...
  \item When work done by the spring force is \emph{negative}, (i.e.\
    $W_e<0$), there is an \emph{increase} in spring potential energy
...
\begin{definition}
  \begin{itemize}
  \item When work done by a conservative force is \emph{positive}, there is a
    \emph{decrease} in the related potential energy by the same amount.
  \item When work done by a conservative force is \emph{negative}, there is
    an \emph{increase} in the related potential energy by the same amount
  \item Positive work by a conservative force transforms the related potential
    energy into kinetic energy; negative work by a conservative force
    transforms kinetic energy into the related potential energy
  \item Work by the a conservative force is always path independent
  \item Only work done by a conservative force $\bm F_g$ can affect its related
...
problem studied in dynamics. Two blocks with masses $m_1$ and $m_2$, stacked
vertically, move to the right without slipping by an external applied force
$\bm F$ applied to $m_2$, as shown in Fig.~\ref{stacked1}.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[scale=.95]
    \fill[pattern=north east lines] rectangle (6,-.2);
    \draw[mass] (1,0) rectangle (4,1.2) node[midway]{$m_2$};
    \draw[thick,fill=pink!30] (1.7,1.2) rectangle (3.3,2) node[midway]{$m_1$};
  ...
  \label{stacked1}
\end{figure}
The coefficient of static friction between the two blocks is $\mu$, but the
contact between $m_2$ and the table is frictionless. The focus of the dynamics
study was to calculate the maximum acceleration $\bm a_\text{max}$ of the
blocks without them sliding against each other, and the maximum applied force
$\bm F_\text{max}$ associated with maximum acceleration. But here, our focus is
to find the work done by the forces between the blocks as the blocks
accelerate.

The free-body diagrams of the blocks are shown in Fig.~\ref{stacked-fbd}.
(The forces highlighted in the same colour are action-reaction pairs.)
\begin{figure}[ht]
  \centering
  \begin{subfigure}{.35\linewidth}
    ...
    \begin{tikzpicture}[vector]
      \fill circle (.1);
      \draw (0,0)--(0,-1) node[below]{$\bm F_{g_1}$};
      \draw (0,0)--(0, 1) node[above,fill=pink!30]{$\bm F_{n_{12}}$};
      \draw (0,0)--(1.5,0) node[right,fill=yellow!30]{$\bm f$};
    \end{tikzpicture}
    \caption{Mass $m_1$ on top}
  \end{subfigure}
  \begin{subfigure}{.35\linewidth}
    \centering
    \begin{tikzpicture}[vector]
      \fill (5,0) circle (.1);
      \draw (4.97,0)--+(0,-1) node[below left]{$\bm F_{g_2}$};
      \draw (5.03,0)--+(0,-1) node[below right,fill=pink!30]{$\bm F_{n_{21}}$};
      \draw (5,0)--+(0,1) node[above]{$\bm F_{n_2}$};
      \draw (5,0)--+(-1.5,0) node[left,fill=yellow!30]{$\bm f$};
      \draw (5,0)--+(2,0) node[right]{$\bm F$};
    \end{tikzpicture}
    \caption{Mass $m_2$ on the bottom}
  \end{subfigure}
  \caption{Free-body diagrams of the stacked masses.}
  \label{stacked-fbd}
\end{figure}
The static friction at the interface between $m_1$ and $m_2$ is $\bm f$, which
acts to the right on $m_1$ and to the left on $m_2$, as required by the third
law of motion. In Chapter~\ref{chapter:dynamics} we aleady know that mass $m_1$
accelerates to the right because of that static friction $\bm f$. Without that
friction $m_1$ would just slide off the back of $m_2$ when $\bm F$ is applied.

On $m_1$, the \emph{only} force doing work is $\bm f$---normal force
$\bm F_{n_{21}}$ and gravitational force $\bm F_{g_1}$ are perpendicular to
motion. The work done by $\bm f$ is \emph{positive}. As $\bm f$ accelerates
$m_1$ to the right, it gains kinetic energy, consistent with the work-energy
theorem (Eq.~\ref{work-energy-theorem}). The change in kinetic energy in $m_1$
as it moves from $x_0\to x_1$ is
\begin{equation*}
  \Delta K_1=\int_{x_0}^{x_1} f\dl x
\end{equation*}
%Note that since we are not assuming whether applied force $\bm F$ is
%constant, we cannot assume whether static friction is constant, and we cannot
On the bottom block $m_2$, as it accelerates to the right, applied force $\bm F$
does \emph{positive} work, while static friction $\bm f$ does \emph{negative}
work.
%At a minimum, we conclude that energy is transferred
%from $m_1$ to $m_2$, but we can do a more thorough (although still very simple)
%analysis to find out how much.
Without static friction, the net force on $m_2$ would have just been $\bm F$,
and the change in kinetic energy moving from $x_0\to x_1$ to the right would
simply be:
\begin{equation*}
  \Delta K_2 = \int_{x_0}^{x_1} F\dl x
  %\quad\quad\text{(no friction)}
\end{equation*}
However, with static friction present, the gain in kinetic energy is reduced
due to the negative work that it does:
\begin{equation*}
  \Delta K_2 = \int_{x_0}^{x_1} F_\text{net}\dl x =
  \int_{x_0}^{x_1} (F-f)\dl x = \int_{x_0}^{x_1} F\dl x -
  \underbracket[1pt]{\int_{x_0}^{x_1} f\dl x}_{\Delta K_1}
\end{equation*}
Work done by static friction reduced the kinetic energy of $m_2$ by the same
amount that is gained by $m_1$. Because the force does positive work on one
block and negative work on the other, We conclude that work done by static
friction transfers kinetic energy from one block to another.

Now, if the applied force $\bm F$ is large enough to overcome static friction,
then it will cause the $m_1$ to slide off $m_2$. We can use this example to
learn about the work done by kinetic friction. For this case, the free-body
diagrams in Fig.~\ref{stacked-fbd} are still valid, but $\bm f$ now represents
\emph{kinetic} friction instead of static friction. As $\bm f$ accelerates
$m_1$ to the right, the gain in kinetic energy as it moves from $x_0\to x_1$ is
\begin{equation*}
  \Delta K_1=\int_{x_0}^{x_1} f\dl x=\mu_km_1g(x_1-x_0)
\end{equation*}
There is no need to integrate because kinetic friction is constant regardless
of the applied force.

With kinetic friction, the bottom block $m_2$ will travel a longer distance,
from $x_0\to x_2$ where $x_2>x_1$, as it accelerates to the right.
Applied force $\bm F$ still does work, while kinetic friction $\bm f$ does
negative work. The gain in kinetic energy is now
\begin{equation*}
  \Delta K_2 = \int_{x_0}^{x_2} F_\text{net}\dl x =
  \int_{x_0}^{x_2} (F-f)\dl x = \int_{x_0}^{x_2} F\dl x -
  \underbracket[1pt]{\mu_km_1g(x_2-x_0)}_{>\Delta K_1}
\end{equation*}
Negative work done by kinetic friction reduced the kinetic energy of $m_2$ by
the more than the kinetic energy gained by $m_1$. So were did the energy go?
A casual observation will show that as the two blocks slide against each other,
both of them are heated up, i.e.\ they increase in tempeature. We therefore
conclude that work done by kinetic friction transfers kinetic energy from
one block to another, and also from kinetic energy into another form of
energy called thermal energy.



\section{Thermal Energy}

Aside from the kinetic energy and potential energies, there is a third category
of energy called the thermal energy.

Consider a container of gas of mass $M$ that oscillates vertically from the
ceiling by a massless spring with stiffness $k$. The container moves at speed
$v$ at a height $h$ above Earth, shown in Fig.~\ref{fig:gas}.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[scale=.7]
    \begin{scope}[thick]
...
\begin{equation*}
  U_g=Mgh
\end{equation*}
where $h$ is the height of the container above Earth, and using the ground
level as the reference $h=0$. of The is also an elastic potential energy of
\begin{equation*}
  U_e=\frac12kx^2
\end{equation*}
stored when the spring is stretched or compressed. But the random motion of
the air molecules in the container also contribute to additional energy,
called the object's \textbf{thermal energy} $E_\text{thermal}$.

Thermal energy of a system of molecules is the sum of all their kinetic and
potential energies at the molecular level:
\begin{equation}
  E_\text{thermal}=K_\text{mol.} + U_\text{mol.}
\end{equation}
As the name suggests, $E_\text{intermal}$ is proportional to molecules'
\textbf{absolute temperature}, measured in \emph{kelvin} (\si\kelvin).
Discussions on thermal energy and the behaviour of gases and solids are part of
a large discipline within physics called \textbf{thermodynamics}, but it is
outside the scope of this book.
\begin{remark}
  For those who are keen to know, for a monatomic ideal gas like helium,
  thermal energy of the gas comes entirely from the kinetic energy from the
  translational motion of the molecules, given by:
  \begin{equation*}
    E_\text{thermal}=\dfrac32Nk_bT
  \end{equation*}
  where $N$ is the number of molecules, $k_b$ is the Boltzmann's constant,
  which is a universal constant governing thermodynamic behaviours. For a
  diatomic gas, thermal energy is given approximately by:
  \begin{equation*}
    E_\text{thermal}\approx\dfrac52Nk_bT
  \end{equation*}
  For solids with a perfect lattice-crystalline structure, like those seen in
  metals, %there are 3 degrees of translational freedom, and 3 degrees of
  %degrees of vibrational freedom, and therefore
  the internal energy is:
  \begin{equation*}
    E_\text{thermal}\approx3Nk_bT
  \end{equation*}
  However, this approximation is only valid for very specific types of material
  structures. The key is that regardless of whether the material is solid,
  liquid or solid, thermal energy is \emph{always} a linear function of
  temperature $T$.
\end{remark}
...
\begin{definition}
  \textbf{Conservation of Energy (lay person's version):} Energy cannot
...
of energy is more nuanced, and can be a boring exercise in bookkeeping:
\begin{definition}
...
\end{definition}
In application, the law of conservation of energy is often a tedius exercise in
...
forces on each other. Because object apply forces on each
other\footnote{This statement may be plainly obvious to some, but not to
...
\item\textbf{Isolated:} All the forces that act on the objects in the system
  ...
  There are either no external forces, or the external forces do not do any
  mechanical work.
...
\subsection{Energies of a System}

The system's energy depend on what objects are part of the system. They include
all the kinetic energies of objects $i$ that may be in motion, the potential
energies stored in the objects, as well as the internal/thermal energies of the
objects. Kinetic and potential energies at the macroscopic levels are 
collectively known as the \textbf{mechanical energy}.
\begin{important-equation}
  E_\text{sys}
  =\underbracket[1pt]{\sum K_i+\sum U_i}_\text{mechanical energy}+
  \sum E_\text{thermal}
  \label{eq:system-energy}
\end{important-equation}
When we solve problems with conservation of energy, we will have consider a
these energies.

The \emph{change} in system energy must therefore be the total of all the
energies in the systems:
\begin{important-equation}
  \Delta E_\text{sys}
  =\sum\Delta K_i+\sum\Delta U_i+\sum\Delta E_\text{thermal}
  \label{eq:system-energy-change}
\end{important-equation}
Substituting the expression from Eq.~(\ref{eq:system-energy-change}) into
...
\section{Gravitational Systems}

\subsection{Free Fall}

Assuming that there are no friction, drag (i.e.\ air resistance, see
Section~\ref{sec:drag}) or other damping forces, a free-falling object (shown
in Fig.~\ref{fig:earth}) forms an isolated system with Earth. 
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[scale=.5]
    \draw[mass] (7.75,0) arc (75:105:30);
...
potential energy ($U_g$) stored between the mass and Earth.
\begin{remark}
  $U_g$ is often described as ``the gravitational potential energy of the
  object''. Strictly speaking, this is incorrect; this is a topic that will be
  studied in more detail in Chapter~\ref{chapter:gravity}. However, this will
  not affect how the calculations is done, and in the opinion of the author,
  poses no major conceptual issue. You are, of course, welcome to disagree.
\end{remark}

The only force that acts in the system is the gravitational force $\bm F_g$,
which, by the third law of motion, acts on \emph{both} the mass and Earth, and
is therefore \emph{internal} to the system. Since there are no external forces,
the system is isolated. As the object falls towards Earth, the gravitational
force is doing positive work on both the mass as well as Earth. Due to the mass
of Earth, its displacement due to this gravitational force is laughably
insignificant. We will therefore only focus on the motion of the mass.

Since gravity is conservative, the positive work done transforms gravitational
potential energy into kinetic energy of the mass by the same amount, and since
$\bm F_g$ is an internal force, the change in the system's energy is zero. The
total change in system energy is zero.
\begin{equation}
  \Delta K + \Delta U_g=0
\end{equation}
...
\begin{example}
  A ball is thrown direction \emph{upwards} with an initial speed $v$ from the
  third-floor balcony of a building, at a height $h$ above the ground. If
  friction and drag can be ignored, with what speed $v'$ would it hit the
  ground?
  
  \textbf{Solution:} This is a simple problem that can be solved using
  kinematics, but we will solve it using conservation of energy instead. Since
  there is no friction and drag, the total mechanical energy is conserved
  between when the ball is thrown, and just before it hits the ground. Setting
  $h=0$ at ground level:
  %, gravitational potential energy is zero ($U_g=mgh=0$) just before the ball
  %hits the ground:
  \begin{displaymath}
    \underbracket[1pt]{K + U_g}_\text{thrown} =
    \underbracket[1pt]{K' + \cancelto{0}{U_g'}}_\text{hitting the ground}
  \end{displaymath}
  Substituting the expressions for $K$ and $U_g$, we have:
  \begin{displaymath}
    \frac12 mv^2 + mgh = \frac12 mv'^2
  \end{displaymath}
  Cancelling mass terms, then solving for $v'$, we have:
  \begin{displaymath}
    v'= \sqrt{v^2+2gh}
  \end{displaymath}
  Our answer is, of course, identical to the 1D kinematic equation---as one
  would expect---but with subtle differences:
  \begin{itemize}
  \item This is a scalar problem, so we didn't bother with defining a positive
    direction, as we would have done when solving a vector problem. We actually
    don't know explicitly the direction of the ball when it hits the ground,
    although it is plainly obvious that it falls downwards.
  \item We cannot find out how long it took to hit the ground.
  \item All we know is the speeds at the end points; we do not know how velocity
    changes during its motion.
  \end{itemize}
...
\begin{example}
  A ball is thrown with an upward angle of $\theta$ with an initial speed of
  $v$ from the same third-floor balcony, a height $h$ above the ground. If
  friction and drag can be ignored, with what speed would it hit the ground?
  \begin{center}
    \begin{tikzpicture}
  ...
  between when the ball is thrown, and just before it hits the ground.
  \begin{displaymath}
    \underbracket[1pt]{K + U_g}_\text{thrown} =
    \underbracket[1pt]{K' + \cancelto{0}{U_g'}}_\text{hitting the ground}
  \end{displaymath}
  Setting $h=0$ at ground level and substituting the expressions for $K$ and
  $U_g$ into the above equation, as we did in the previous example, we have:
  \begin{displaymath}
    \frac12 mv^2 + mgh = \frac12 mv'^2
  \end{displaymath}
  Again, cancelling mass terms, then solving for $v'$:
  numerical values:
  \begin{displaymath}
    v'= \sqrt{v^2+2gh}
  \end{displaymath}
  This is, of course, the exact same answer as the previous example. Previously,
  solving this problem using 2D kinematics would have required us to split the
  problem into horizontal ($x$) and vertical ($y$) directions. We would have
  to find the vertical velocity when the ball hits the ground, then combine
  with the horizontal velocity to find the \emph{speed}. Using conservation of
  energy allows us to solve a scalar problem instead of a vector problem
  allowed us to solve this problem is a fraction of the time, and without
  worrying about whether we have used the correct vector directions.
\end{example}
%Talk about why we don't include thermal energy in the examples.



\section{Arbitrary Ramp}

Assuming that there is no friction, drag or other damping forces present,
energy is also conserved for an object sliding down an arbitrarily-shaped ramp:
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
...
only gravity does work ($\bm F_n$ is always perpendicular to motion). The
sum of the kinetic energy of the mass ($K$) and the gravitational potential
energy ($U_g$) stored between the mass and Earth is constant
\begin{equation}
  K+U_g=\text{constant}
\end{equation}
...
\begin{example}
  A block slides down a arbitrarily shaped ramp with an initial speed $v$ from
  a height $h$ above the ground. If friction and drag can be ignored, with what
  speed $v'$ would it be when the block reaches the ground?
  \begin{center}
    \begin{tikzpicture}[scale=.7]
      \draw[thick] (0,4) to[out=-30,in=180] (3,1) to[out=0,in=180] (5,3)
      to[out=0,in=170] (8,0) to[out=-10,in=180] (10,0);
      \draw[mass,rotate around={-60.5:(1,2.93)}] (1,2.93) rectangle +(.6,.6);
      \draw[vector,red,rotate around={-60:(1.4,2.8)}] (1.4,2.8)--+(1.4,0)
      node[right]{$\bm v$};
    \end{tikzpicture}
  \end{center}
  \textbf{Solution:} This is a simple problem using conservation of energy, but
  impossible to solve using kinematics and dynamics, because \emph{we are never
  told what the shape of the ramp is}! Since there is no friction and drag, the
  total mechanical energy is conserved throughout the entire motion of the
  block. Setting $h=0$ at ground level:
  \begin{displaymath}
    \underbracket[1pt]{K + U_g}_\text{start} =
    \underbracket[1pt]{K' + \cancelto{0}{U_g'}}_\text{end}
  \end{displaymath}
  Substituting the expressions for $K$ and $U_g$, we have:
  \begin{displaymath}
    \frac12 mv^2 + mgh = \frac12 mv'^2
  ...
  \begin{displaymath}
    v'= \sqrt{v^2+2gh}
  \end{displaymath}
  Our answer is, of course, identical to the two previous gravitational
  problems, and the same as using 1D kinematic equation. We can note that using
  conservation of energy is really the only viable method to solving this
  problem.
\end{example}
...
point, thus allowing it to vibrate in the horizontal direction, as shown in
...
elastic potential energy stored in the spring ($U_e$). Several \emph{external}
...
any work because they are orthogonal to the motion of the mass. Therefore
...
\begin{example}
  ...
  \begin{center}
    \pic{.3}{spring-example1}
  \end{center}
  \textbf{Solution:} I will get around to it when I have time!
\end{example}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.9]
    ...
    \draw[thick] (-1,0)--(1,0);
    \draw[gray] (0,-5) arc (-90:-45:5) node[right]{path};
    \draw[gray] (0,-5) arc (-90:-135:5);
    \draw[<->] (0,0)--+(0,-5)  node[midway,left]{$\ell$};
    \draw[<->] (-.62,0)--(-.62,-5*cos{25}) node[midway,left]{$\ell\cos\phi$};
    \draw[dashed] (-1,-5*cos{25})--+(3.5,0);
    \draw[<->,red,thick] (-.62,-5)--(-.62,-5*cos{25}) node[midway,left]{$h$};
    \begin{scope}[rotate=25]
      \draw[very thick] (0,0)--(0,-5) node[midway,right]{$\ell$};
      \draw[mass] (0,-5) circle (.2) node[right=4]{$m$};
      \fill[red] (0,-5) circle (.07);
      \draw[red,vector] (0,-5)--(0,-3.5) node[left]{$\bm F_T$};
      \draw[red,vector,rotate around={-25:(0,-5)}] (0,-5)--(0,-6.3)
      node[below]{$\bm F_g$};
    \end{scope}
    \draw[axes] (0,-2) arc (270:295:2) node[midway,below]{$\phi$};
  \end{tikzpicture}
  \caption{A simple pendulum system is an isolated system}
...
the system, there are only two forces act on the pendulum bob: the tension force
$\bm F_T$, and the gravitational force $\bm F_g$. Gravitational force is an
internal force (because it acts on both the pendulum bob and Earth), while
tension force is an \emph{external} force. However, since $\bm F_T$ always acts
perpendicular to the motion of the pendulum bob, it does not do any work, and
the system is therefore an isolated system. The total energy of the system is
therefore constant. The conservation of energy is the same as the gravitational
systems previously:
\begin{equation}
  K + U_g = \text{constant}
\end{equation}

%When a simple pendulum of length $\ell$ is deflected by an angle $\phi$, the
%change in height is can be calculated using basic trigonometry, as shown in
%Fig.~\ref{fig:height-change}
%\begin{equation}
%  h=\ell-\ell\cos\phi=\ell(1-\cos\phi)
%\end{equation}
%\begin{figure}[ht]
%  \centering
%  \begin{tikzpicture}
...
%      \shade[ball color=red] (0,-5) circle (.2);
%      \begin{scope}[rotate=35]
%        \draw[thick] (0,0)--(0,-5) node[midway,right]{$\ell$};
%        \shade[ball color=red] (0,-5) circle (.2);
%      \end{scope}
%      \draw[dashed] (-0,-5)--+(-1,0);
%      \draw[axes] (0,-1.5) arc(-90:-55:1.5) node[midway,below]{$\phi$};
%  \end{tikzpicture}
%  \caption{Change in height when the pendulum is deflected by angle $\phi}
%  \label{fig:height-change}
%\end{figure}
%
%
%\begin{example}
...
\subsection{Why use conservation of energy\ldots and why not?}
If solving motion problems using the law of energy conservation is so much
easier, why don't we use it all the time?

Advantages:
\begin{itemize}
\item Scalar: there are no vectors and no need to define directions
\item Algebra: calculus can be avoided in most cases
\item Fast!
\end{itemize}

Disadvantages:
\begin{itemize}
\item Cannot tell you direction of motion
\item Cannot tell you the total time
\item Cannot tell you the acceleration of objects in many cases
\item For spring-mass and pendulum systems: cannot tell you why
  object oscillates
\end{itemize}
...
%TL\section*{Exercise Questions}
%TL
%TL\subsection*{Multiple-Choice Questions}
%TL
%TL%\begin{multicols*}{2}
%TL\begin{enumerate}[leftmargin=12pt,itemsep=4pt]
%TL\item A \SI1{\kilo\gram} ball is thrown vertically downward from a
%TL  \SI{50}{\metre} high tower with an initial speed of \SI4{\metre\per\second}.
%TL  Just before striking the ground, the speed of the ball is
%TL  \SI{20}{\metre\per\second}. The energy lost to air friction is most nearly
%TL  %\begin{choices}
%TL  %  \choice\SI{101}\joule
%TL  %  \choice\SI{210}\joule
%TL  %  \choice\SI{308}\joule
%TL  %  \choice\SI{406}\joule
%TL  %  \choice\SI{508}\joule
%TL  %\end{choices}
%TL
%TL  %TL%  \textbf{Questions \ref{q:pq1}--\ref{q:pq2}}:
%TL\item A \SI2{\kilo\gram} projectile is launched with a speed of
%TL  \SI{20}{\metre\per\second} from horizontal ground at an angle of \ang{37}
%TL  to the horizontal, as shown. Point $P$ is at the top of the path, and point
%TL  $Q$ is at the end of the path, just before the projectile again reaches the
%TL  ground. The kinetic energy of the projectile at point $P$ is
%TL  %  \cpic{.35}{symprojectile}
%TL  %    \begin{choices}
%TL  %      \choice\SI{108}\joule
%TL  %      \choice\SI{225}\joule
%TL  %      \choice\SI{256}\joule
%TL  %      \choice\SI{400}\joule
%TL  %      \choice\SI{525}\joule
%TL  %    \end{choices}
%TL    
%TL\item The kinetic energy of the projectile at point $Q$ from the previous
%TL  question is
%TL  %    \begin{choices}
%TL  %      \choice\SI{108}\joule
%TL  %      \choice\SI{225}\joule
...
%TL  %    \end{choices}
%TL
%TL\item If a projectile thrown directly upward reaches a maximum height
%TL  $h$ and spends a total time in the air of $T$, then returning to the
%TL  original location, the average power of the gravitational force is
%TL  %\begin{choices}
%TL  %  \choice $P=2mgh/T$
%TL  %  \choice $P=-2mgh/T$
%TL  %  \choice 0
...
%TL  %\end{choices}
%TL    
%TL\item Given that the constant net force on an object and the object's 
%TL  displacement, which of the following quantities can be calculated?
%TL  %\begin{choices}
%TL  %  \choice the net change in the object's velocity
%TL  %  \choice the net change in the object's mechanical energy
%TL  %  \choice the average acceleration
%TL  %  \choice the net change in the object's kinetic energy
%TL  %  \choice the net change in the object's potential energy
%TL  %\end{choices}
%TL    
%TL\item A conservative force acting on an object varies with the equation
%TL  $F(x)=-3x^2-2x-4$, where force is in newtons and displacement is in metres.
%TL  The potential energy at $x=\SI2\metre$ is
%TL  %\begin{choices}
%TL  %  \choice zero
%TL  %  \choice\SI{20}\joule
%TL  %  \choice\SI{40}\joule
%TL  %  \choice\SI{-20}\joule
%TL  %  \choice\SI{-40}\joule
%TL  %\end{choices}
%TL    
%TL\item The only force acting on an object is given by the equation
%TL  $F(x)=2-4x$, where the force is measured in newtons and position in meters.
%TL  What is the change in the object's kinetic energy as it moves from $x=2$ to
%TL  $x=1$?
%TL  %\begin{choices}
%TL  %  \choice +\SI4\joule
%TL  %  \choice \SI{-4}\joule
%TL  %  \choice +\SI2\joule
%TL  %  \choice \SI{-2}\joule
%TL  %  \choice +\SI8\joule
%TL  %\end{choices}
%TL    
%TL\item The potential energy of an object varies with the equation
%TL  $U(x)=2x^2+x-6$, where force is in newtons and displacement is in meters. A
%TL  force $F$ vs.\ displacement $x$ graph would yield which of the following?
%TL  %\begin{choices}
%TL  %  \choice A straight, horizontal line
%TL  %  \choice A parabola
%TL  %  \choice An exponential decay curve
%TL  %  \choice A straight line with a positive slope
%TL  %  \choice A straight line with a negative slope
%TL  %\end{choices}
%TL    
%TL\item A particle of mass $m$ moves according to the displacement
%TL  equation $x=2t^{5/2}$. The kinetic energy of the particle as a function of
%TL  time is
%TL  %\begin{choices}
%TL  %  \choice $10mt^{5/2}$
%TL  %  \choice $10mt^{3/2}$
%TL  %  \choice $\dfrac{25}2mt^3$
%TL  %  \choice $5mt^2$
%TL  %  \choice $2mt^{3/2}$
%TL  %\end{choices}
%TL    
%TL\item An electron travels in a circle around a hydrogen nucleus at a
%TL  very high speed. The work done by the electrostatic force acting on the
%TL  electron after one complete revolution is
%TL  %\begin{choices}
%TL  %  \choice zero
%TL  %  \choice positive
%TL  %  \choice negative
%TL  %  \choice equal to the kinetic energy of the electron
%TL  %  \choice equal to the potential energy of the electron
%TL  %\end{choices}
%TL    
%TL\item An object is moved from rest at point $P$ to rest at point $Q$ in
%TL  a gravitational field. The net work against the gravitational field depends
%TL  on the
%TL  %\begin{choices}
%TL  %  \choice mass of the object and the positions of $P$ and $Q$
%TL  %  \choice mass of the object only
%TL  %  \choice positions of $P$ and $Q$ only
%TL  %  \choice length moved between points $P$ and $Q$
%TL  %  \choice coefficient of friction
%TL  %\end{choices}
%TL  
%TL\item Two blocks of mass $m_A$ and $m_B$ are connected by a string that
%TL  passes over a light pulley. The mass of $A$ is larger than the mass of $B$.
%TL  The speed of mass $A$ just before reaching the floor is:
%TL  \begin{center}
%TL    \begin{tikzpicture}[scale=.7,thick]
%TL      \draw circle(1);
%TL      \draw (-1,0)--(-1,-1.5);
%TL      \draw (-1.5,-1.5) rectangle (-.5,-2.5) node[midway]{$A$};
%TL      \draw (1,0)--(1,-3);
%TL      \draw (1.5,-3) rectangle (.5,-4)  node[midway]{$B$};
%TL      \draw (-1.8,-2.5)--(-2.2,-2.5);
%TL      \draw[<->] (-2,-2.5)--(-2,-5.5) node[midway,fill=white]{$D$};
%TL      \fill[gray!50] (-2.3,-5.7) rectangle (2,-5.5);
%TL      \draw (-2.3,-5.5)--(2,-5.5);
%TL    \end{tikzpicture}
%TL  \end{center}
%TL  %    \begin{choices}
%TL  %      \choice $\sqrt{\dfrac{2(m_A-m_B)}{m_A+m_B}gD}$
%TL  %      \choice $\sqrt{\dfrac{2(m_A+m_B)}{m_A-m_B}gD}$
%TL  %      \choice $\sqrt{\dfrac{2m_A}{m_A+m_B}gD}$
%TL  %      \choice $\sqrt{\dfrac{2m_B}{m_A+m_B}gD}$
%TL  %      \choice $\sqrt{\dfrac{2m_A}{m_B}gD}$
%TL  %    \end{choices}
%TL
%TL%TL    \uplevel{
%TL%TL      \textbf{Questions \ref{q:well1}--\ref{q:well2}}: Consider the potential
%TL%TL      energy function shown below.
%TL%TL      \cpic{.28}{potential-well}
%TL%TL    }
%TL%TL
%TL%TL    \question Assuming that no non-conservative forces are present, if a
%TL%TL    particle of mass $m$ is released from position $x_0$, what is the maximum
%TL%TL    speed it will achieve?
%TL%TL    \label{q:well1}
%TL%TL    \begin{choices}
%TL%TL      \choice $\sqrt{4U_0/m}$
%TL%TL      \choice $\sqrt{2U_0/m}$
%TL%TL      \choice $\sqrt{U_0/m}$
%TL%TL      \choice $\sqrt{U_0/2m}$
%TL%TL      \choice The particle will achieve no maximum speed but instead will
%TL%TL      continue to accelerate indefinitely.
%TL%TL    \end{choices}
%TL%TL    
%TL%TL    \question Which of the following is the most accurate description of the
%TL%TL    system introduced in the previous question?
%TL%TL    \label{q:well2}
%TL%TL    \begin{choices}
%TL%TL      \choice A stable equilibrium
%TL%TL      \choice An unstable equilibrium
%TL%TL      \choice A neutral equilibrium
%TL%TL      \choice A bound system
%TL%TL      \choice There is a linear restoring force
%TL%TL    \end{choices}
%TL%TL    \columnbreak
%TL
%TL  \item A pendulum bob of mass $m$ is released from rest as shown in the
%TL    figure below. What is the tension in the string as the pendulum swings
%TL    through the lowest point of its motion?
%TL    \begin{center}
%TL      \begin{tikzpicture}
%TL        \draw[thick,dashed] (0,0)--(0,-3.5);
%TL        \begin{scope}[rotate=60]
%TL          \draw[very thick] (0,0)--(0,-4);
%TL          \fill (0,-4) circle (.13) node[right]{$\;m$};
%TL          \draw[<->,thick] (.5,0)--(.5,-4) node[midway,above right]{$l$};
%TL        \end{scope}
%TL        \draw[<->,thick] (0,-1) arc (270:330:1) node[pos=.6,below]{\ang{60}};
%TL      \end{tikzpicture}
%TL    \end{center}
%TL%    \begin{choices}
%TL%      \choice $T=\dfrac12mg$
%TL%      \choice $T=mg$
%TL%      \choice $T=\dfrac32mg$
%TL%      \choice $T=2mg$
%TL%      \choice None of the above
%TL%    \end{choices}
%TL
%TL  \item A force is applied to a block of mass $m$ at a downward angle of
%TL    $\theta$ to the vertical as shown. The block moves with a constant speed
%TL    across a rough floor for a distance $x$. The work done by the applied force
%TL    on the block is
%TL    \begin{center}
%TL      \begin{tikzpicture}
%TL        \fill[gray!50] (0,-.15) rectangle (5,0);
%TL        \draw[thick] (0,0)--(5,0);
%TL        \draw[thick] (1.5,0) rectangle (3,1);
%TL        \draw[<-,thick] (3,1)--(4.3,2.3);
%TL        \draw[thick,dashed] (2.5,2.3)--(4.3,2.3)--(4.3,.8);
%TL        \draw[thick] (4.3,1.6) arc (270:225:.7)
%TL        node[midway,below left]{$\theta$};
%TL      \end{tikzpicture}
%TL    \end{center}
%TL%    \begin{choices}
%TL%      \choice $Fx\sin\theta$
%TL%      \choice $Fx\cos\theta$
%TL%      \choice $Fmx\sin\theta$
%TL%      \choice $Fmx\cos\theta$
%TL%      \choice zero
%TL%    \end{choices}
%TL
%TL%TL%    \uplevel{
%TL%TL%      \textbf{Questions \ref{sphere1}--\ref{sphere2}}:      
%TL%TL%      A small block rests on the top of a smooth sphere of radius $R$ when it
%TL%TL%      is given a light tap so that it just begins sliding on the sphere. When
%TL%TL%      the block reaches the angle $\theta$, it loses contact with the surface
%TL%TL%      of the sphere.
%TL%TL%      \begin{center}
%TL%TL%        \begin{tikzpicture}[scale=.9]
%TL%TL%          \draw[thick]circle (2);
%TL%TL%          \fill circle (.05);
%TL%TL%          \draw[axes] (0,0)--(0,2) node[midway,left]{$R$};
%TL%TL%          \draw[thick] (-.2,2) rectangle(.2,2.4);
%TL%TL%          \begin{scope}[rotate around={-40:(0,0)}]
%TL%TL%            \draw[thick,dashed] (0,0)--(0,2);
%TL%TL%            \draw[thick] (-.2,2) rectangle (.2,2.4);
%TL%TL%            \draw[vectors] (0,2.2)--(.8,2.2) node[right]{$\mb{v}$};
%TL%TL%          \end{scope}
%TL%TL%          \draw[thick] (0,.6) arc (90:50:.6) node[midway,above]{$\theta$};
%TL%TL%        \end{tikzpicture}
%TL%TL%      \end{center}
%TL%TL%    }
%TL%TL%
%TL%TL%    \question The kinetic energy of the block as it leaves the surface of the
%TL%TL%    sphere is
%TL%TL%    \label{sphere1}
%TL%TL%    \begin{choices}
%TL%TL%      \choice $mgR$
%TL%TL%      \choice $mgR\cos\theta$
%TL%TL%      \choice $mgR\sin\theta$
%TL%TL%      \choice $mg(R-R\cos\theta)$
%TL%TL%      \choice $mg(R-R\sin\theta)$
%TL%TL%    \end{choices}
%TL%TL%
%TL%TL%    \question The speed of the block as it leaves the surface of the sphere is
%TL%TL%    \label{sphere2}
%TL%TL%    \begin{choices}
%TL%TL%      \choice $\sqrt{2g}m$
%TL%TL%      \choice $\sqrt{2gR}m$
%TL%TL%      \choice $\sqrt{2gR\cos\theta}$
%TL%TL%      \choice $\sqrt{2gR(1-\cos\theta)}$
%TL%TL%      \choice $\sqrt{2gR(1-\sin\theta)}$
%TL%TL%    \end{choices}
%TL    
%TL  \item A machine can lift large weights according to the power equation
%TL    $P(t)=4t^3+3t^2-2$, where power is in watts and time is in seconds. The
%TL    energy expended by the machine from $t=0$ to $t=\SI{10}{\second}$ is
%TL    %\begin{choices}
%TL    %  \choice \SI{1260}\joule
%TL    %  \choice \SI{3630}\joule
%TL    %  \choice \SI{9240}\joule
%TL    %  \choice \SI{10980}\joule
%TL    %  \choice \SI{18150}\joule
%TL    %\end{choices}
%TL  \end{enumerate}
%TL%\end{multicols*}
%TL%TL\newpage
%TL%TL
%TL%TL%\genfreetitle{3}{WORK AND ENERGY}{3}
%TL%TL%\genfreedirections
%TL\subsection*{Problem-Solving Questions}
%TL
%\begin{enumerate}
...
%TL        \draw[vectors] (0,0)--(34,0);
%TL        \draw[vectors] (0,0)--(0,27);
