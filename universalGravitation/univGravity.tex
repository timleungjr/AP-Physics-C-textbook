...
force $\mathbf F_{12}$ on $m_2$, then $m_2$ likewise also exerts a reaction force of
$\mathbf F_{21}=-\mathbf F_{12}$ on $m_1$. The two forces are equal in magnitude and
...
\begin{common-question}
  ...
  the situation for $r=0$ will never arise; $F_g$ is never be infinite. From a
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=.4]
    ...
    \draw[vector,blue,rotate=45] (.7,0)--(2.5,0) node[right]{$\mathbf F_1$};
    \shade[ball color=blue] (5,5) circle (.8) node[white]{$m_1$};

    \draw[vector,green,rotate=-45] (-.7,0)--(-2,0) node[left]{$\mathbf F_2$};
    \shade[ball color=green] (-5,5) circle (.6) node[white]{$m_2$};

    \draw[vector,violet,rotate=45](-.7,0)--(-3.5,0) node[left]{$\mathbf F_3$};
    \shade[ball color=violet] (-5,-5) circle (1.2) node[white]{$m_3$};

    \draw[vector,magenta,rotate=-45](.7,0)--(2.7,0) node[right]{$\mathbf F_4$};
...
\begin{equation}
  \boxed{
    ...
    =-GM\left[\sum_{i=1}^N\frac{m_i}{r_i^2}\hat{\mathbf r_i}\right]
  }
\end{equation}
where $r_i$ is the distance from $M$ to $m_i$, and $\hat{\mathbf r_i}$ is the
...
\begin{equation}
  \boxed{
    \mathbf F=-\int\dl\mathbf F=-GM\int\frac{\dl m}{r^2}\hat{\mathbf r}
  }
\end{equation}
where $\hat{\mathbf r}$ is the outward radial direction from $\dl m$ towards $M$.
...
\end{equation*}
where $\mathbf g$ is the acceleration due to gravity at the location of $m$.
This equation is always
correct as long as we have the correct magnitude and direction of $\mathbf g$.
...
gravitation to find $\mathbf g$:
\begin{equation*}
  F_g
  =\underbrace{\left[\frac{Gm_1}{r^2}\right]}_{=g}m_2
  =m_2 g
\end{equation*}
The vector field function $\mathbf g$ is known as the \textbf{gravitational field}
generated by point mass $m_1$. It is a function of the magnitude of the source
mass $m_s$, and the distance $r$ from it:
\begin{equation}
  \boxed{
    \mathbf g(m_s,\mathbf r)=-\frac{Gm_s}{r^2}\hat{\mathbf r}
  }
  \label{eq:g}
\end{equation}
The direction of the gravitational field is the inward radial direction
$-\hat{\mathbf r}$, i.e.\ $\mathbf g$ points towards the source mass. The gravitational
...
field at any point of interest is the vector sum of all the fields $\mathbf g_i$
from those masses:
\begin{equation}
  \boxed{\mathbf g
    =\sum_i\mathbf g_i
    =-G\left[\sum_{i=1}^N\frac{m_i}{r_i^2}\hat{\mathbf r}_i\right]
  }
\end{equation}
where $r_i$ is the distance from the point of interest to $m_i$ and $\hat{\mathbf r}_i$
...
\begin{equation}
  \boxed{
    \mathbf g
    =\int\dl\mathbf g
    =-G\int\frac{\dl m}{r^2}\hat{\mathbf r}
  }
\end{equation}
where $\hat{\mathbf r}$ is the outward radial direction from $\dl m$ towards the point
of interest. This integral may be difficult to compute if the geometry is complicated.

%When a mass $m$ is placed inside a gravitational field $\mathbf g$, it
%experiences a gravitational force given by the familiar equation:
The gravitational field $\mathbf g$ doesn't \emph{do} anything itself, until another mass
$m$ is placed inside the field. Then, the gravitational field applies a  force
$\mathbf F_g$ on that mass $m$, given by the familiar equation:
  %enters the field. Then, $m$ experiences a gravitational force 
  %proportional to $m$ and $\mathbf g$, 
\begin{equation*}
  \mathbf F_g=m\mathbf g
\end{equation*}
...
\item The effects of that gravitational field, which is always $\mathbf F_g=m\mathbf g$
\end{enumerate}
In the example in Fig.~\ref{fig:g-example1}:
\begin{itemize}
\item The masses $m_1$ and $m_2$ generate gravitational fields
  $\mathbf g_1$ and $\mathbf g_2$. The gravitational fields extend throughout space, and
...
\item At position $P$, the total field is the vector sum of both masses, i.e.
  $\mathbf g_\text{tot}=\mathbf g_1+\mathbf g_2$. At this point,
...
\item When a mass $m$ is placed inside the gravitational field, it experiences a
  gravitational force of $\mathbf F_g=m\mathbf g_\text{tot}$. The force is in the same
  direction as $\mathbf g_\text{tot}$.
\item Since the gravitational field is also the acceleration due to gravity, if 
  ...
  $\mathbf a=\mathbf g_\text{tot}$. If gravity is not the only force, then the mass's
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
	...
	\draw[violet] circle (.4) node[white]{$\mathbf m_1$};
	...
	\draw[orange] (-10,2) circle (.25) node[orange]{$\mathbf m_2$};
    \foreach \theta in {0,30,...,359}{
...
i.e.\ as $\mathbf g(\mathbf x,t)$ for every position $\mathbf x$ in a space that we are
...
\begin{equation}    
  \boxed{
    ...
    \begin{cases}
      \mathbf 0 & \text{if}\;\;r<R\\
      -Gm_1m_2/r^2\hat{\mathbf r} & \text{otherwise}
    \end{cases}
  }
\end{equation}
It also means that gravitational field is also \emph{zero} inside the shell.
That $\mathbf g_\text{inside}=\mathbf 0$ can be calculated by integrating the fields
...
\begin{equation}
  \oint\mathbf g\cdot\dl\mathbf A=-4\pi GM_\text{encl}
\end{equation}
...
\begin{equation*}
  \oint_\mathcal{S}\mathbf E\cdot\dl\mathbf A=\frac{q_\text{encl}}{\epsilon_0}
\end{equation*}
...
field $\mathbf E$ with gravitational field $\mathbf g$, enclosed charge $q_\text{encl}$
...
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
    \draw[axes] (0,0)--(0,3) node[above]{$|\mathbf g|$};
...
\end{figure}
%\begin{itemize}
%\item The direction of $\mathbf g$ is towards the centre of the object that
%  created it
%\item Field lines do not tell the intensity (i.e.\ magnitude) of $\mathbf g$,
%  only the direction
%\end{itemize}
%
...
by integrating the work ($W_g$) done by the gravitational force ($\mathbf F_g$):
\begin{equation*}
  W_g =\int \mathbf F_g\cdot\dl\mathbf r
  =-\int_{r_0}^{r_1}\frac{Gm_1m_2}{r^2}\hat{\mathbf r}\cdot\dl\mathbf r
...
$\mathbf F_g$ is the \emph{negative gradient} of the gravitational potential
energy $U_g$. The direction of $\mathbf F_g$ would therefore always point from
high to low potential energy. For a point mass, we can express this
relationship by:
\begin{equation}
  \mathbf F_g = -\nabla U_g = -\diffp{U_g}r\hat{\mathbf r}
\end{equation}
As is the case in Chapter~\ref{chapter:energy}, an object moving along the
direction of $\mathbf F_g$ is always decreasing in $U_g$, and the direction of
$\mathbf F_g$ is called the \emph{steepest descent direction}; it is the shortest
path to decrease $U_g$. Objects travelling perpendicular to $\mathbf F_g$ would
have a constant $U_g$.

Knowing that $\mathbf F_g$ and $\mathbf g$ only differ by a constant (mass $m$), we
can also relate gravitational field to potential energy by the gradient
operator:
\begin{equation}
  \mathbf g=-\nabla V_g=-\diffp{V_g}r\hat{\mathbf r}
  \quad\text{where}\quad
  V_g=\frac{U_g}m
\end{equation}
%We already know that the direction of $\mathbf g$ is the same as $\mathbf F_g$, i.e.
%\begin{itemize}
%\item The direction of $\mathbf g$ is the shortest path to decrease $U_g$ 
%\item Objects travelling perpendicular to $\mathbf g$ has constant $U_g$
...
\item Gravity is a \emph{central force}, in that the direction of the
  gravitational force $\mathbf F_g$ is always in the $-\hat{\mathbf r}$ direction, as
  shown in Fig.~\ref{central-force}. Therefore
  $\mathbf r\times\mathbf F=\mathbf\tau_g=\mathbf 0$. Gravity doesn't generate any torque
  about the sun, and angular momentum $\mathbf L$ is constant.
\end{enumerate}
\begin{figure}[!ht]
  \centering
  \begin{tikzpicture}[scale=1.2]
    \draw[vector,blue] (0,0)--(2,3) node[midway,left]{$\mathbf r$};
    %\draw[vector] (0,0)--(2,3) node[midway,left]{$\mathbf r$};
    \begin{scope}[rotate around={-33.7:(2.03,3)}]
      \draw[vector,red](2.03,3)--(2.03,1.5) node[midway,below right]{$\mathbf F_g$};
    \end{scope}
    \draw[vector] (2,3)--(3,2.8) node[right]{$\mathbf v$};
...
of Earth with some initial velocity $\mathbf v$.
\begin{figure}[ht]
  \centering
  \begin{subfigure}{.4\textwidth}
    ...
    \end{tikzpicture}
    \caption{With a low initial velocity, the object falls onto the planet
...
$\mathbf v_\text{orbit}$---which is tangent to the cicular path--- is called
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.8]
    ...
    \begin{scope}[rotate=50]
      ...
      \draw[vector,violet] (3,0)--+(0,-2) node[right]{$\mathbf v_\text{orbit}$};
...
$\mathbf F_c=m\mathbf a_c$, and substituting the gravitational force $F_g$ into
...
all uniform circular motion, the direction of $\mathbf v_\text{orbit}$ is tangent
...
\begin{example}
  Determine the escape velocity for a \SI{1.60e4}{\kilo\gram} rocket leaving
  ...
  \SI{11}{\kilo\metre\per\second}. Consider that a commercial airliner flies at
...
The equation for the escape speed is based on the object have a \emph{constant}
mass, which is \emph{not} the case for a rocket going into space. In reality, a
rocket will be travelling at supersonic speeds, and therefore have an enormous
amount of drag force acting on it. Additionally, the mass of a rocket is mostly
in its fuel, while the mass of the structure, engines, and payload comprises of
a very small percentage of the weight at launch.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.4]
    ...
    \draw[vector] (-8,0)--+(0,4) node[above]{$\mathbf v_1$};
    \draw[vector] (4,0)--+(0,-6) node[below]{$\mathbf v_2$};
    \draw[vector,blue] (-8,0)--(-5,0) node[above]{$\mathbf F_g$};
    \draw[vector,red] (4,0)--(1,0) node[above]{$\mathbf F_g$};
    %\shade[ball color=red] (4,0) circle (1) node[white]{$m_2$};
    %\shade[ball color=blue] (-8,0) circle (.7) node[white]{$m_1$};
...
\begin{example}
  \textbf{Binary System Example:} Two stars of mass $m_1$ and $m_2=2m_1$ orbit
  each other. From Chapter~\ref{chapter:cm}, we know that the orbital distances
  are related by $r_1=2r_2$. To find the orbital speed of $m_1$, Setting the
  gravitational force to be the centripetal force:
  \begin{equation*}
    F_c=ma_c\quad\rightarrow\quad \frac{Gm_1m_2}{(r_1+r_2)^2}=\frac{m_1v_1^2}{r_1}
  \end{equation*}
  Note that unlike the case in Section~\ref{sec:orbital-speed}, the orbital
  radius $r_1$ is \emph{not} the same as the distance between the distance
  between the stars ($r_1+r_2$). Now setting $m_2=2m_1$ and $r_2=r_1/2$, the
  equation becomes:
  \begin{equation*}
  ...
  \end{equation*}
  This would mean that both stars have the same orbital period.
\end{example}
...
depending on the magnitude of $\mathbf v$ at $P$:
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
    ...
    \draw[vector] (0,4)--(1.5,4) node[above]{$\mathbf v$};
...
\begin{figure}[!ht]
  ...
  \begin{tikzpicture}[scale=.8]
    ...
    \begin{scope}[rotate around={\angle:({sqrt(\a*\a-\b*\b)},0)}]
      ...
      node[pos=.45,below]{$\mathbf r$} node[left]{$\mathbf v_r$};
      \draw[->](7.65,0)--(7.65,.85) node[below]{$\mathbf v_\theta$};
...
\end{figure}
\begin{itemize}
\item\textbf{Angular velocity} $\mathbf v_\theta$. The presence of
  $\mathbf v_\theta$ means that there is a centripetal acceleration $a_c$ toward
  $M$. The direction of the acceleration is perpendicular to the direction of
  $\mathbf v_\theta$, and toward the sun:
  \begin{equation}
    a_c=-r\omega^2\hat{\mathbf r}
  \end{equation}
\item\textbf{Radial velocity} $\mathbf v_r$. If this velocity is non-zero and
  ...
  direction:
  \begin{equation}
    a_r=\diff{v_r}t\hat{\mathbf r}=\diff[2]rt\hat{\mathbf r}
  \end{equation}
\end{itemize}
...
To begin, recognize that for a planet with a velocity vector $\mathbf v$ that is
not co-linear with the displacement vector $\mathbf r$ will sweep out an
infinitesimal area $\dl\mathbf A$ (Fig.~\ref{fig:dA})
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[scale=1.3]
    \fill[gray!45](0,0)--(2,3)--(3,2.8)--cycle;
    \node at (1.7,2) {$\dl\mathbf A$};
    \draw[->,thick] (0,0)--(2,3)
    node[midway,left]{$\mathbf r$} node[pos=.95,right]{$\alpha$};
    \draw[->,thick] (2,3)--(3,2.8) node[midway,above]{$\dl\mathbf r$};
    \draw[dashed] (3,2.8)--(0,0);
    \fill circle (.07) node[right]{Sun};
    \fill (2,3) circle (.035) node[left]{Planet};
  \end{tikzpicture}
  \caption{Infinitesimal area $\dl\mathbf A$ swept by a planet as it moves
    $\dl\mathbf r$ in orbit.}
  \label{fig:dA}
\end{figure}
as it moves in orbit by an infinitesimal amount $\dl\mathbf r$, which can be
...
or in vector form:
\begin{equation}
  \dl\mathbf A=\frac12\mathbf r\times \dl\mathbf r
\end{equation}
The direction of $\dl\mathbf A$, in this case, points into the page.\footnote{The
...
\begin{equation}
  \diff{\mathbf A}t
  =\frac12\mathbf r\times\diff{\mathbf r}t
  =\frac12\mathbf r\times\mathbf v
\end{equation}
We can express $\mathbf r\times\mathbf v$ in terms of angular momentum. Since
angular momentum is defined as $\mathbf L=m(\mathbf r\times\mathbf v)\;\rightarrow\;
\mathbf r\times\mathbf v=\mathbf L/m$. Since gravity is a central force, angular
momentum is a constant, i.e:
\begin{equation}
  \boxed{\diff{\mathbf A}t=\frac12(\mathbf r\times\mathbf v)=\frac{\mathbf L}{2m}=
    \text{constant}
  }
  \label{constant}
\end{equation}
...
%conservation of angular momentum $\mathbf L=m(\mathbf r\times\mathbf v)$ (gravity is a
%central force).
%
%
...
%%    \begin{tikzpicture}[scale=1.3]
%%      \fill[gray!45](0,0)--(2,3)--(3,2.8)--cycle;
%%      \node at (1.7,2) {$d\mathbf{A}$};
%%      \draw[->,thick](0,0)--(2,3)
%%      node[midway,left]{$\mathbf r$} node[pos=.95,right]{$\alpha$};
%%      \draw[->,thick](2,3)--(3,2.8) node[midway,above]{$d\mathbf r$};
%%      \draw[dashed](3,2.8)--(0,0);
%%      \fill (0,0) circle (.07) node[right]{\tiny\text{Sun}};
%%      \fill (2,3) circle (.035) node[above]{\tiny\text{Planet}};
%%    \end{tikzpicture}
%
%%    The infinitesimal area $d\mathbf{A}$ swept out by an object (such as a planet)
%%    as it moves in orbit by an infinitesimal amount $d\mathbf r$ is given by:
%%    
%%    \eq{-.4in}{
...
%%    \eq{-.15in}{
%%      \frac{d\mathbf{A}}{dt}
%%      =\frac12\mathbf r\times\frac{d\mathbf r}{dt}
%%      =\frac12\mathbf r\times\mathbf{v}
%%    }
%
...
%  velocity} $\mathbf v_r$ and \textbf{angular velocity} $\mathbf v_\theta$.
%\begin{center}
...
%      node[pos=.45,below]{$\mathbf r$}
%      node[left]{$\mathbf v_r$};
%      \draw[vector] (7.65,0)--(7.65,.85) node[below]{$\mathbf v_\theta$};
%      \fill (7.65,0) circle (.1) node[above]{$m$};
%    \end{scope}
%    \draw[axes]({sqrt(\a*\a-\b*\b)+.5},0) arc(0:\angle:.5)
...
%
%\begin{itemize}
%\item $\mathbf v_\theta$ means a centripetal acceleration toward $M$
%\item Changes in $\mathbf v_r$ (i.e.\ acceleration in the radial direction)
