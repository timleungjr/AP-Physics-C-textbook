...
\fcolorbox{black}{magenta!10}{
  ...
  \begin{minipage}{.97\linewidth}
    ...
    that point masses do not actually exist, and that even an electron has a
    finite size. Therefore the situation for $r=0$ will never arise; $F_g$ is
    never be infinite. From a practical point of view, whether a mass can be
...
\begin{itemize}
\item The masess $m_1$ and $m_2$ generate gravitational fields
  ...
  as point masses, and the gravitationalfields are given by equation Eq.~\ref{eq:g}.
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{.24\textwidth}
    \centering
    \begin{tikzpicture}
      \node at (0,0) {\pic{.75}{../universalGravitation/earth-1200px}};
  ...
  \begin{subfigure}{.24\textwidth}
    ...
    \begin{tikzpicture}
      \node at (0,0) {\pic{.75}{../universalGravitation/earth-1200px}};
      \fill[red] (0,1.7) circle (.05);
      \draw[function,->] (0,1.7) arc (90:-40:1.5 and 1.5);
      \draw[dashed,gray,thick] circle (1.7);
    \end{tikzpicture}
...
\fcolorbox{black}{yellow!10}{
  \small
  \begin{minipage}{.97\linewidth}
    \textbf{Example:} Determine the escape velocity for a
    ...
    \SI{11}{\kilo\metre\per\second}. Consider that a commercial airliner flies at about
    \SI{13}{\kilo\metre} in altitude, the rocket will reach that height in just over
    1 second. The rocket's mass, although is given to you, is not required in solving
    the problem.
  \end{minipage}
} 

The equation for the escape speed is based on the object have a \emph{constant}
mass, which is \emph{not} the case for a rocket going into space. In reality, a rocket
will be travelling at supersonic speeds, and therefore have an enormous amount of drag
force acting on it. Additionally, the mass of a rocket is mostly in its fuel, while
the mass of the structure, engines, and payload comprises of a very small percentage of
the weight at launch.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=.4]
    ...
    \draw[vector] (-8,0)--+(0,4) node[above]{$\bm v_1$};
    \draw[vector] (4,0)--+(0,-6) node[below]{$\bm v_2$};
    \draw[vector,blue] (-8,0)--(-5,0) node[above]{$\bm F_g$};
    \draw[vector,red] (4,0)--(1,0) node[above]{$\bm F_g$};
...
\fcolorbox{black}{violet!5}{
  ...
  \begin{minipage}{.97\textwidth}
    ...
    each other. From Chapter~\ref{chapter:cm}, we know that the orbital
    distances are related by $r_1=2r_2$. To find the orbital speed of $m_1$, Setting
    the gravitational force to be the centripetal force:
    \begin{equation*}
      F_c=ma_c\quad\rightarrow\quad \frac{Gm_1m_2}{(r_1+r_2)^2}=\frac{m_1v_1^2}{r_1}
    \end{equation*}
    Note that unlike the case in Section~\ref{sec:orbital-speed}, the orbital radius
    $r_1$ is \emph{not} the same as the distance between the distance between the 
    stars ($r_1+r_2$). Now setting $m_2=2m_1$ and $r_2=r_1/2$, the equation becomes:
    \begin{equation*}
...
\item{\color{blue} If the object is moving slower than the orbital speed, i.e.\
  $v<v_\text{orb}$, theobject would lose altitude, arriving at point $P''$ on
...
\begin{figure}[!ht]
  ...
  \begin{tikzpicture}[xscale=1.5]
    ...
    \foreach \x in {-1,...,5}{
      \draw[thick] (\x+2,0)--(\x+2,-.2) node[below]{$10^{\x}$};
      \draw[thick] (0,\x+2)--(-.2,\x+2) node[left]{$10^{\x}$};
    }
    \draw[thick] (0,0)--(7,7);
