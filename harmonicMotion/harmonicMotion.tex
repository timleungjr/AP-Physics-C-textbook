...
$\mathbf F_g=m\mathbf g$, normal force $\mathbf F_n$, and spring force $\mathbf F_s$.
\begin{figure}[hbt]
  \centering
...
\end{figure}
Since there is no vertical motion, $\sum\mathbf F_\text{vertical}=0$, $\mathbf F_g$ and
$\mathbf F_n$ cancel out. The net force is due only to spring force
$\mathbf F_s=-k\mathbf x$ in the horizontal direction. This is true when the spring is
...
force is called a \emph{restoring force} because $\mathbf F_S$ always points
...
\begin{common-question}
  ...
  $x(t)=r\cos(\theta)=r\cos(\omega t+\theta_0)$ which is the same form as
  Eq.~\ref{eq:ode1}. The ``radius'' of the circular motion is the amplitude
  $A$ of the harmonic motion. 
  \begin{center}
...
\begin{remark}
  In calculus classes, you are generally taught that the solution to any
  ODE is the linear combination of \emph{all} possible solutions. In other
  words, since both sine and cosine functions are possible solutions, our
  solution to the ODE must be in the form
  \begin{equation*}
    x(t)=c_1\cos(\omega t)+c_2\sin(\omega t)
  \end{equation*}
  where $c_1$ and $c_2$ are coefficients based on initial conditions $x_0=x(0)$
  and $v_0=v(0)$. Trigonometric identities can be used to show that
...
force $\mathbf F_g=m\mathbf g$ which acts along the direction of motion. In this case,
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=1.3]
    ...
    \draw[vector,red] (1,2)--(1,1.2) node[right]{$\mathbf F_g$};
    \draw[vector,red] (1,2)--(1,2.8) node[right]{$\mathbf F_s$};
...
case, two forces act on the mass: weight $\mathbf F_g$ and tension $\mathbf F_T$, as
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[rotate=20]
      ...
      \draw[vector,red] (0,-5)--(0,-3) node[left]{$\mathbf F_T$};
...
\begin{common-question}
  \textbf{How small is a ``small angle''?}
  What constitutes a small angle depends on what tolerance (i.e.\ the number of
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=1.3]
    ...
    \begin{scope}[vector,red]
      \draw (3.5,1)--(3.5,0) node[right]{$\mathbf F_g$};
      \draw (3.5,1)--(3.5,2) node[right]{$\mathbf F_n$};
      \draw (3.5,1)--(2.5,1) node[above]{$\mathbf F_s$};
      \draw (3.5,1)--(4.5,1) node[above]{$\mathbf F_D$};
    \end{scope}
...
oscillator, the motion of the damped oscillator is not strictly
periodic\footnote{Let's call it \emph{quasi}-periodic.}. The natural
frequency $\omega'$ for the damped oscillator is shifted from the undamped case
...
%a rate of:
%\begin{equation}
%  P=\diff Et=\mathbf F_D\cdot\mathbf v=-bv^2
%\end{equation}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=1.3]
    ...
    \begin{scope}[vector,red]
      ...
      \draw (3.5,1)--(3.5,2) node[right]{$\mathbf F_n$};
      \draw (3.5,1)--(2.5,1) node[above]{$\mathbf F_s$};
      \draw (3.5,.95)--(2,.95) node[below]{$\mathbf F_a$};
      \draw (3.5,1)--(4.5,1) node[below]{$\mathbf F_D$};
    \end{scope}
    \fill[red] (3.5,1) circle (.075);
  \end{tikzpicture}
...
\item a \textbf{steady-state solution} which does not depend on the initial
  condition, only on the forcing function $\mathbf F_a$. Solving for the
