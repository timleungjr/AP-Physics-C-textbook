...
when there is no friction or damping forces. However, applying the law of
...
friction and other damping forces, as shown in
...
\begin{figure}[hbt]
  ...
  \end{tikzpicture}
  \caption{A horizontal spring-mass system with no friction and damping forces}
...
\begin{equation*}
  \ddot x + \frac kmx=0 %\diff[2]xt + \frac kmx=0
\end{equation*}
}. The solution to the equation is a function $x(t)$ where the second time
derivative $\ddot x$ looks like $x$ but with a negative sign. The only
...
\emph{must} be a periodic motion. Starting with this general form:
\begin{equation}
  \boxed{
  ...
  \label{eq:gen-form}
\end{equation}
where $A$ is the amplitude of the oscillation, $\omega_0$ is the angular
frequency of the motion, called \textbf{natural frequency}, and $\phi$ is a
...
\fcolorbox{black}{pink!20}{
  \begin{minipage}{.97\textwidth}
    \textbf{Common Question: When should we use cosine, and when should be use
      sine?}

    If oscillation begins at amplitude $A$ at $t=0$, the cosine function is
    preferred so that we can set $\phi=0$. Conversely, if oscillation begins
...
\begin{align}
  v(t)&=\dot x=-A\omega_0\sin(\omega_0 t+\phi)\\
  a(t)&=\ddot x=-A\omega_0^2\cos(\omega_0 t+\phi)=-\omega_0^2x
\end{align}
...
the constant $-\omega_0^2$. Substituting expressions of $x(t)$ and $a(t)$ back
into Eq.~\ref{eq:ode1}, we find that the ODE is satisfied if the natural
frequency $\omega_0$ is related to the spring constant and mass by:
\begin{equation}
  \boxed{
...
The period ($T_0$) and frequency ($f_0$) of the simple harmonic oscillator are
then given by:
\begin{equation}
...
\fcolorbox{black}{pink!20}{
  \small
  \begin{minipage}{.97\textwidth}
    \textbf{Common Question: Is $\omega_0$ the same angular velocity term from
      rotational motion?} Yes, it most certainly is. In essence, the simple
    harmonic motion is the projection of a uniform circular motion onto an
    axis. If angular position $\theta(t)=\omega t+\omega_0$, then
    $x(t)=r\cos(\theta)=r\cos(\omega t+\omega_0)$ which is the same form as
    Eq.~\ref{eq:ode1}.
    \begin{center}
...
\fcolorbox{black}{cyan!5}{
  \small
  \begin{minipage}{.97\textwidth}
    \textbf{Side Note \#1:} Generally in calculus classes, you would be taught
    that the solution to any ODE is the linear combination of \emph{all}
    possible solutions, i.e.:
    \begin{equation*}
      x(t)=c_1\cos(\omega t)+c_2\sin(\omega t)
    \end{equation*}
    Where $c_1$ and $c_2$ are coefficients based on initial position $x_0$ and
    velocity $v_0$. Trigonometric identities can be used to show that this
    equation is identical to Eq.~\ref{eq:gen-form}:
    \begin{align*}
    ...
    \end{align*}
    From an even broader discussions on mathematics, all the possible solutions
    to the ODE are functions that are \emph{orthogonal} to each other.
  \end{minipage}
}
...
\fcolorbox{black}{cyan!5}{
  \small
  \begin{minipage}{.97\textwidth}
    \textbf{Side Note \#2:} Another function that we can try is the exponential
    function, where its derivatives is related to the function itself:
    \begin{align}
      \dot x &=\omega e^{\omega t}\\
      \ddot x &=\omega^2 e^{\omega t}=\omega^2x\label{eq:2nd-derivative}
    \end{align}
    ...
    \begin{align}
      ...
      \ddot x&=i^2\omega^2 e^{i\omega t}=-\omega^2 e^{i\omega t}=-\omega^2x
    \end{align}
...
For vertical spring-mass systems (Fig.~\ref{fig:vertical-spring-mass}), we must
consider weight $\bm F_g=m\bm g$ as well.
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}[scale=1.3]
...
\begin{figure}[ht]
  ...
  \fcolorbox{black}{pink!20}{
    \begin{minipage}{.97\textwidth}
      \textbf{Common Question: How small is a ``small angle''?} What
      constitutes a small angle depends on what tolerance (i.e.\ the number of
      significant figures) is needed in the answer. When we plot the difference
      between $y=\theta$ and $y=\sin\theta$, we can see that both functions are
      very similar near $\theta=0$. However, sine function levels off at
      $\theta=\frac\pi2$ and the linear function obviously does not. The
      percentage error is shown on the right. For a \SI1{\percent} error, the
      angle of deflection should be kept to less than \SI4\degree.
      \begin{center}
        \begin{tikzpicture}
          \begin{axis}[
              width=2.7in,
...
\fcolorbox{black}{yellow!10}{
  ...
  \begin{minipage}{.97\linewidth}
    ...
    supporting the elevator break and all  of the brakes and safety features
    fail simultaneously. The elevator plunges into free fall. The young girl is
...
drag, $n=2$. Of course our choice of using $n=1$ will not be correct for every
problem, but it is sufficiently accurate for a wide range of situations.}
...
$\omega_0$ based on the damping factor $b$:
\begin{equation}
  \boxed{
...
natural frequency $\omega_0$ or the damped natural frequency $\omega'$.
\begin{figure}[t]
  \centering
  \begin{tikzpicture}[scale=1.3]
    \draw[gray,mass] (3,.5) rectangle (4,1.5);
...
highest when $\omega_a\approx\omega\approx\omega_0$, and that smaller the
...
\end{equation}
When $\omega_a=\omega_0$ is substituted into the phase shift expression, the
...
\end{equation}
For a lightly damped system, $\omega_a\approx\omega\approx\omega_0$. Also, the
