...
\end{equation*}
}. The solution to Eq.~\ref{eq:ode1} is a function $x(t)$ where the second time
derivative $\diff[2]xt$ looks like $x$, but with a negative sign. The only
two functions that can satisfy this equation are the sinusoidal functions:
$\sin(t)$ and $\cos(t)$, which means that the motion along the horizontal
direction \emph{must} be periodic. Starting with this general form:
\begin{important-equation}
...
\end{important-equation}
And the second derivative gives us acceleration of the mass:
\begin{important-equation}
...
\begin{remark}
  ...
  $\theta_0=0$. If oscillation begins at amplitude $x=A$ at $t=0$, the cosine
...
\begin{remark}
  ...
  \begin{align*}
    ...
    x(t) &=r\sin(\theta)=r\cos(\omega t+\theta_0)
  \end{align*}
...
system:
\begin{equation}
  \boxed{\theta(t)=\theta_\text{max}\cos(\omega t-\theta_0)}
\end{equation}
where $\theta_\text{max}$ is the maximum deflection (amplitude), and $\theta_0$ is a
phase shift based on the initial condition of the pendulum. (If the oscillation
begins at amplitude, then $\theta_0=0$) The natural frequency of the oscillation
$\omega$ is given by:
\begin{equation}   
  \boxed{
...
not as simple as the simple harmonic oscillator in Eq.~\ref{eq:ode1}. The
...
\begin{equation}
  \boxed{x(t)=A\cos(\omega_a t+\theta_0)}
  \label{eq:gen-solution3}
\end{equation}
%(For those who are interested, Appendix X shows how the steady-state solution
%is derived.)
The amplitude of the oscillation $A$ depends on the driving frequency
$\omega_a$ of the forcing function:
\begin{important-equation}
...
The phase constant $\theta_0$ in Eq.\ref{eq:gen-solution3} also depends on the
...
$\theta_0=\pi/2$. Taking derivative of $x(t)$ for velocity $v(t)$, and
substituting $\theta_0=\pi/2$:
\begin{equation}
  v(t)=\dot x
