...
The unit for angular position is a \emph{radian} (rad).

If motion is confined to the $xy$-plane, then $\theta$ is the standard angle:
$\theta$ is positive when measured counterclockwise from the $x$-axis, and
negative when it is measured clockwise. In the example shown in
...
Analogous to the relationship between position $\bm x$, displacement
$\Delta\bm x$ and velocity $\bm v$, for circular motion, the instantaneous
\textbf{angular velocity} (also known as \textbf{angular frequency})
$\omega(t)$, is the time derivative of the angular position\footnote{Of course,
we can also take the time derivative of angular displacement as wel, as
$\theta(t)$ and $\Delta\theta(t)$ only differ by the (constant) initial position
$\theta_0$. And the derivative of a constant is zero.} measured in
\emph{radian per second} (\si{\radian\per\second}). Angular velociry is also a
function of time:
\begin{important-equation}
    \omega(t)=\diff{\theta}t
\end{important-equation}
...
$\omega(t)$. The difference, of course, is that the integral for angular
position, is indefinite, and the integral for angular displacement is definite.
\begin{important-equation}
...
\end{equation}
For \emph{uniform} circular motion, $\omega$ is constant, $\alpha=0$, and
therefore $a_t=0$.
...
(i.e.\ the angular direction $\hat{\bm\theta}$), tangential acceleration is
caused by the \textbf{tangential force} $\bm F_t$:
\begin{important-equation}
  \bm F_t=m\bm a_t=mr\alpha\hat{\bm\theta}
\end{important-equation}
while in the radial ($\hat{\bm r}$) direction, centripetal acceleration
(directed in the inward radial direction $-\hat{\bm r}$) is caused by a
\textbf{centripetal force} $\bm F_c$:
\begin{important-equation}
  \bm F_c
  =m\bm a_c
...
\begin{center}
  \pic{.55}{circularMotion/car-circular2}
\end{center}
%  {
%   \footnotesize
...
\begin{remark}
  Including tangential forces \emph{greatly} increases the complexity of the
  problem. As a car moves along the track, it experiences a drag force, and
  rolling resistance from the deformation of the tires. Both of these forces
  act in opposition to motion. To maintain speed, a component of the static
  friction must also point in the direction of motion. For front-wheel drive
  car, static friction force points forward, along the direction of
  motion, while in the rear wheels, static friction points backward, opposite
  to motion! Assessing the magnitudes of the actual tangential forces is not a
  simple process, so our analysis will definitely be a simplication.
\end{remark}
...
%    %    \end{center}        
  ...
  component is the centripetal force. Therefore, we will have a free-body
  diagram that looks like this:
  \begin{center}
    \begin{tikzpicture}[scale=.6]
      \fill circle (.07);
      \draw[thick] (0,0)--(-4,-2) node[midway,left]{$\ell$};
      \draw[thick,dashed] (0,-2) ellipse (4 and 1);
      \shade[poscharge] (-4,-2) circle (.3);
    \end{tikzpicture}
  \end{center}
...
\begin{figure}[ht]
  ...
  \begin{subfigure}{.47\textwidth}
    \centering
    \pic{.9}{circularMotion/Autoroute-A20}
    \caption{Highway off ramp}
  \end{subfigure}
  \begin{subfigure}{.47\textwidth}
    \centering
    \pic{.9}{circularMotion/185644-main-track-turn}
    \caption{Racetrack}
    \label{fig:daytona}
  \end{subfigure}
  ...
  \begin{subfigure}{.47\textwidth}
    \centering
    \pic{.9}{circularMotion/67587c526761b}
    \caption{Cycling velodrome}
    \label{fig:milton-velodrome}
  \end{subfigure}
  \begin{subfigure}{.47\textwidth}
    \centering
    \pic{.9}{circularMotion/Wall-of-Death-2024}
  ...
  \begin{tikzpicture}[scale=1.1]
    \begin{scope}[rotate=20,thick]
      \node[rotate=20] at (0,0) {
        \pic{.2}{circularMotion/bmw-back-small-shadowy}
      };
      \draw (-3,-1)--+(6,0);
...
\begin{example}
  A roller coaster car is on a track that forms a circular loop, of radius $R$,
  in the vertical plane. If the car is to maintain contact with the track at
  the top of the loop (generally considered to be a good thing), what is the
  minimum speed that the car must have at the bottom of the loop? Ignore air
...
\begin{example}
  A stone of mass $m$ is attached to a light strong string and whirled in a
  \emph{vertical} circle of radius $r$. At the exact bottom of the path, the
  tension of the string is three times the weight of the stone. The stone's
  speed at that point is:
%  \begin{enumerate}[(A)]
%  \item $2\sqrt{gR}$
%  \item $\sqrt{2gR}$
%  \item $\sqrt{3gR}$
