...
%
\chapter{Kepler's Laws of Planetary Motion}
...
\begin{equation}
  \bm F_g=-\frac{Gm_1m_2}{r^2}\hat{\bm r}
\end{equation}
are crucial to
...
\item Gravity is a \emph{central} force, in that
  \begin{itemize}[nosep]
  \item gravitational force $\bm F_g$ is always in the $-\hat{\bm r}$ direction
    (i.e.\ $\bm F\times\bm r=\bm 0$), shown in
    Fig.~\ref{central-force}, therefore gravity does not generate any torque,
  \item angular momentum $\bm L$ about the sun is therefore constant
  \end{itemize}
\end{enumerate}
\begin{figure}[!ht]
  \centering
  \begin{tikzpicture}[scale=1.3]
    ...
    node[midway,left]{$\bm r$};
    \draw[->,thick] (0,0)--(2,3) node[midway,left]{$\bm r$};
    \begin{scope}[rotate around={-33.7:(2.03,3)}]
      \draw[->,ultra thick,red](2.03,3)--(2.03,1.5)
      node[midway,below right]{$\bm F_g$};
    \end{scope}
    \draw[->,thick] (2,3)--(3,2.8) node[right]{$\bm v$};
